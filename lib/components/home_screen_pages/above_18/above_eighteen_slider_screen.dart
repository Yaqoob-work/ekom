...
// //==============================================================================
// // SECTION 1: COMMON CLASSES (Keep these if this is a separate file)
// //==============================================================================
...
// // Renamed from GenreMoviesScreen to AboveEighteenSliderScreen
// //==============================================================================
...
//   //=================================================
//   // SECTION 2.1: DATA FETCHING (MODIFIED FOR ADULT MOVIES)
//   //=================================================
...
//         // Call 1: Fetch Genres (UPDATED: data_for = adultmovies)
//         https.post(
//           genreUrl,
//           headers: headers,
//           body: json.encode({
//             "data_for": "adultmovies", // <--- Changed from "" to "adultmovies"
//             "network_id": int.tryParse(widget.tvChannelId) ?? 0
//           }),
//         ),
//         // Call 2: Fetch Movies (UPDATED: data_for = adultmovies added to ensure correct content)
//         https.post(
//           movieUrl,
//           headers: headers,
//           body: json.encode({
...
//             "data_for": "adultmovies" // <--- Added this to ensure matching content
//           }),
//         ),
//       ]);
...
//           });
//         } else {
//           // If status is false, it might mean no data for "adultmovies"
//           print("API returned status false. Genre Msg: ${json.decode(genreRes.body)['message']}");
...
//     // Priority Sorting for "Web Series"
...
//   void initState() {
//     super.initState();
//     _isDisposed = false; // Initialize flag
//     SecureUrlService.refreshSettings();
//     _sliderPageController = PageController();
//     _searchButtonFocusNode = FocusNode();
...
//                 MaterialPageRoute(
//                     builder: (context) => YoutubeWebviewPlayer(
//                         videoUrl: rawUrl, name: content.name))).catchError((e) => null);
//           } else {
//             await Navigator.push(
//               context,
//               MaterialPageRoute(
...
//   void _focusFirstMovieItemWithScroll() {
//     if (_isDisposed || _movieFocusNodes.isEmpty) return;
//     if (_movieScrollController.hasClients) {
//       _movieScrollController.animateTo(0.0,
//           duration: AnimationTiming.fast, curve: Curves.easeInOut);
//     }
...
//   static List<Color> gradientColors = [accentRed, accentOrange, accentPink];
// }
...
// class ProfessionalMovieLoadingIndicator extends StatelessWidget {
//   final String message;
//   const ProfessionalMovieLoadingIndicator({Key? key, required this.message}) : super(key: key);
...
//               gradient: LinearGradient(colors: ProfessionalColors.gradientColors),
//             ),
...
//           Text(
//             message,
//             style: const TextStyle(color: ProfessionalColors.textPrimary, fontSize: 16, fontWeight: FontWeight.w500),
//           ),
//         ],
//       ),
//     );
...
//               _isLoading
//                   ? const Center(
//                       child: ProfessionalMovieLoadingIndicator(message: 'Loading Content...'))
//                   : _errorMessage != null
//                       ? _buildErrorWidget()
//                       : _buildPageContent(),
//               if (_isVideoLoading && _errorMessage == null)
...
//                     color: Colors.black.withOpacity(0.8),
//                     child: const Center(
//                       child: ProfessionalMovieLoadingIndicator(message: 'Loading Details...'),
//                     ),
//                   ),
//                 ),
//             ],
...
//   Widget _buildBackgroundOrSlider() {
//     if (_contentSliders.isNotEmpty) {
//       return MovieBannerSlider(
//         sliders: _contentSliders,
//         controller: _sliderPageController,
//         onPageChanged: (index) {
//           if (mounted) setState(() => _currentSliderPage = index);
//         },
...
//                   fit: BoxFit.cover,
//                   placeholder: (context, url) => Container(color: ProfessionalColors.cardDark),
...
//   final List<ContentSlider> sliders;
...
//               errorWidget: (context, url, error) => Container(color: ProfessionalColors.surfaceDark),
//             );
//           },
//         ),
//         Container(
...
class _AboveEighteenSliderScreenState extends State<AboveEighteenSliderScreen> {
  ...
  Future<void> _fetchDataForPage() async {
    ...
    try {
      ...
      if (results[0].statusCode == 200 && results[1].statusCode == 200) {
        final genreData = GenreResponse.fromJson(json.decode(results[0].body));
  ...
  void _playContent(Movie content, int index) async {
    if (_isVideoLoading || _isDisposed || !mounted) return;
    setState(() => _isVideoLoading = true);
    ...
    return MasterSliderLayout<Movie>(
      ...
      onFilterSelected: (idx) {
        setState(() {
          ...
          _displayList = _moviesByGenre[_genres[idx]] ?? [];
          _displayList.shuffle();
        });
      },
      onSearch: _onSearch,

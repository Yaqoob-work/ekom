...
//   // Pehle register karein, phir focus check karein
//   _setupFocusProvider();
// }
...
//     if (mounted) {
//       final focusProvider = Provider.of<FocusProvider>(context, listen: false);
//       const String myId = 'subVod'; // Is row ka identifier
...
//           // 2. ✅ CRITICAL FIX: Agar Dashboard isi row par focus lana chahta hai
...
//         // Error state mein bhi focus check karein
//         if (focusProvider.lastFocusedIdentifier == myId) {
...
// //             context.read<ColorProvider>().resetColor();
// //             FocusScope.of(context).unfocus();
// // // 1. Dashboard ko switch karo (Next Page: RECENTLY ADDED)
...
//   // // Agle row (RECENTLY ADDED) ka identifier set karein
//   // // Taki wo load hote hi focus pakad le
//   // fp.updateLastFocusedIdentifier('manageMovies');
...
// // ✅ ==========================================================
// // VOD SERVICE (Direct API Call)
// // ==========================================================
// class HorizontalVodService {
//   static Future<String> fetchRawData() async {
//     String authKey = SessionManager.authKey;
//     var url = Uri.parse(SessionManager.baseUrl + 'getNetworks');
//     final response = await https.post(url, headers: {'auth-key': authKey, 'Content-Type': 'application/json', 'Accept': 'application/json', 'domain': SessionManager.savedDomain}, body: json.encode({"network_id" :"" ,"data_for" : ""})).timeout(const Duration(seconds: 30));
//     if (response.statusCode == 200) {
...
//     if (mounted) setState(() { _loadingState = LoadingState.loading; _error = null; });
//     try {
//       final freshRawData = await HorizontalVodService.fetchRawData();
//       if (freshRawData.isNotEmpty) {
//         final parsedData = await compute(_parseAndSortVod, freshRawData);
//         _applyDataToState(parsedData);
//       } else {
...
//   // ✅ UPDATE: Scroll logic calculates exact position based on margin to slide items to the left
//   void _scrollToPosition(int index) {
//     if (!_scrollController.hasClients) return;
    
//     // Total Width = banner width + (left margin 15 + right margin 15)
//     final double itemTotalWidth = bannerwdt + 30; 
...
//   // ✅ UPDATE: Large right padding added so the last items can slide all the way to the start
//   Widget _buildHorizontalVodList(double screenWidth, double screenHeight) {
//     return FadeTransition(
//       opacity: _listFadeAnimation,
//       child: SizedBox(
...
//           //   right: screenWidth * 0.7 // Space to allow last item to scroll to start
//           // ),
//           padding: EdgeInsets.only(
...
//       focusNode: focusNode,
//       onFocusChange: (hasFocus) {
//         // ✅ Direct state update accurately for the focused text logic
//         if (mounted) {
//           setState(() {
//             _isSectionFocused = _vodFocusNodes.values.any((node) => node.hasFocus); 
//             if (hasFocus) {
//               focusedIndex = index;
//             }
//           });
...
//   // ✅ UPDATE: Main Header Title Widget (Dynamic Name and Colors)
//   Widget _buildProfessionalTitle(double screenWidth) {
//     String selectedVodName = "";
    
//     bool isRowFocused = _vodFocusNodes.values.any((node) => node.hasFocus);
    
//     if (focusedIndex != -1 && focusedIndex < _vodList.length && isRowFocused) {
//       selectedVodName = _vodList[focusedIndex].name.toUpperCase();
//     }
...
//       child: Container(
//         // padding: EdgeInsets.symmetric(horizontal: screenWidth * 0.025),
//         padding: EdgeInsets.only(left: screenWidth * 0.16, right: screenWidth * 0.025),
//         child: Row(
//           crossAxisAlignment: CrossAxisAlignment.center,
//           children: [
//             ShaderMask(
...
//               child: const Text(
//                 "CONTENTS", 
//                 style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.w800, letterSpacing: 2.0)
//               ),
//             ),
            
//             if (selectedVodName.isNotEmpty) ...[
//               const SizedBox(width: 15),
//               const Text(
//                 "|", 
//                 style: TextStyle(fontSize: 22, color: Colors.grey, fontWeight: FontWeight.w300)
//               ),
//               const SizedBox(width: 15),
//               Expanded(
...
//                     key: ValueKey<String>(selectedVodName),
//                     child: Text(
//                       selectedVodName,
//                       maxLines: 1,
//                       overflow: TextOverflow.ellipsis,
...
// // ✅ Fire TV Style Professional TV Show Card (Center Scaled & Black Unfocused Text)
// class ProfessionalHorizontalVodCard extends StatefulWidget {
//   final HorizontalVodModel HorizontalVod;
//   final FocusNode focusNode;
//   final VoidCallback onTap;
...
//     // ✅ Image popup scale (Massive 1.28x scale)
//     _scaleAnimation = Tween<double>(
//       begin: 1.0,
//       end: 1.28, 
//     ).animate(CurvedAnimation(
...
//       widget.onColorChange(Colors.white);
//       HapticFeedback.lightImpact();
//     } else {
//       _scaleController.reverse();
...
//     return Container(
//       width: bannerwdt,
//       // ✅ Margin expanded to accommodate the larger 1.28x scale
...
//         crossAxisAlignment: CrossAxisAlignment.center,
//         children: [
//           // ✅ Only scale the poster to keep it vertically centered
//           AnimatedBuilder(
//             animation: _scaleAnimation,
//             builder: (context, child) {
//               return Transform.scale(
//                 scale: _scaleAnimation.value,
//                 child: _buildProfessionalPoster(screenWidth),
//               );
//             },
//           ),
...
//   Widget _buildProfessionalPoster(double screenWidth) {
//     return Container(
//       height: bannerhgt,
//       decoration: BoxDecoration(
...
//                 fit: StackFit.expand,
//                 children: [
//                   _buildHorizontalVodImage(screenWidth),
//                   _buildGenreBadge(),
//                 ],
//               ),
//             ),
...
//   Widget _buildHorizontalVodImage(double screenWidth) {
//     return SizedBox(
//       width: double.infinity,
...
//   Widget _buildGenreBadge() {
//     if (widget.HorizontalVod.genres == null || widget.HorizontalVod.genres!.isEmpty) {
...
class HorzontalVod extends StatefulWidget {
  const HorzontalVod({super.key});
  @override
  State<HorzontalVod> createState() => _HorzontalVodState();
}

class _HorzontalVodState extends State<HorzontalVod> with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;
  ...
  Future<List<CommonContentModel>> fetchContentsAPI() async {
    ...
    if (response.statusCode == 200) {
      ...
      return activeData.map((item) {
        String badge = 'CONTENTS';
        String rawGenres = (item['genres'] ?? '').toString().toLowerCase();
        if (rawGenres.contains('news')) badge = 'NEWS';
        else if (rawGenres.contains('sports')) badge = 'SPORTS';
        else if (rawGenres.contains('entertainment')) badge = 'ENTERTAIN';

        return CommonContentModel(id: item['id'].toString(), title: item['name'] ?? 'Unknown', imageUrl: item['logo'] ?? '', badgeText: badge, originalData: item);
      }).toList();
    } else { throw Exception('Failed to load contents'); }
  }

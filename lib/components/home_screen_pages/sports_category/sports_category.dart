...
//       if (mounted) {
//         final focusProvider = Provider.of<FocusProvider>(context, listen: false);
//         const String myId = 'sports'; // Is page ka identifier
...
//           // _buildSportsItem ke andar onKey handler mein:
// // ✅ 3. DOWN ARROW UPDATE (Next Page ke liye)
// else if (key == LogicalKeyboardKey.arrowDown) {
//   // setState(() { focusedIndex = -1; _hasReceivedFocus = false; });
//   // context.read<ColorProvider>().resetColor();
...
//   // // Agle page ka identifier ('kids_show') set karein
//   // fp.updateLastFocusedIdentifier('religiousChannels'); 
...
class _ManageSportsState extends State<ManageSports> with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;
  ...
  Future<List<CommonContentModel>> fetchSportsAPI() async {
    ...
    } else { throw Exception('Failed to load sports'); }
  }
  ...
  Future<void> _onItemTap(CommonContentModel item) async {
    ...
    await Navigator.push(context, MaterialPageRoute(builder: (context) => SportsSliderScreen(initialNetworkId: int.parse(item.id))));
  }

  Future<void> _onViewAllTap() async {
    await Navigator.push(context, MaterialPageRoute(builder: (context) => const SportsSliderScreen(initialNetworkId: null)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SmartCommonHorizontalList(
      sectionTitle: "SPORTS",
      titleGradient: const [ProfessionalColorsForHomePages.accentBlue, ProfessionalColorsForHomePages.accentGreen],
      accentColor: ProfessionalColorsForHomePages.accentBlue,
      placeholderIcon: Icons.sports_soccer, badgeDefaultText: 'SPORTS',
      focusIdentifier: 'sports',

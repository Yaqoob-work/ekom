...
class BannerSlider extends StatefulWidget {
  final Function(bool)? onFocusChange; 
  final FocusNode focusNode; 
  final bool isPlanExpiring; // ✅ NEW
  final int daysLeft;        // ✅ NEW

  const BannerSlider({
    Key? key,
    this.onFocusChange,
    required this.focusNode,
    this.isPlanExpiring = false, // ✅ NEW
    this.daysLeft = 0,           // ✅ NEW
  }) : super(key: key);
  ...
  Widget _buildBannerSlider() {
    return Stack(
      children: [
        ...
        // ✅ PERMANENT PLAN EXPIRY WARNING LAYER (BOTTOM OF BANNER)
        if (widget.isPlanExpiring)
          Positioned(
            bottom: 25,
            left: screenwdt * 0.20,
            right: screenwdt * 0.20,
            child: IgnorePointer( // Ignore pointer to not block any banner clicks below
              child: Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const Icon(Icons.warning_amber_rounded, color: Colors.yellow, size: 28),
                    const SizedBox(width: 12),
                    Text(
                      'Your plan expires in ${widget.daysLeft} ${widget.daysLeft == 1 ? 'day' : 'days'}. Please renew immediately.',
...
// Update _handleWatchNowTap to be async
  Future<void> _handleWatchNowTap() async {
    if (selectedContentId != null && bannerList.isNotEmpty) {
      
      // ✅ VERIFY PLAN BEFORE PLAYING AND UPDATE UI
      bool isSafe = await _verifyPlanStatusLocally();
      if (!isSafe) return;
  ...
  // ✅ ADDED LOCAL VERIFICATION TO BANNER SLIDER
  Future<bool> _verifyPlanStatusLocally() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // ✅ DISPATCH NOTIFICATION SO DASHBOARD UPDATES ITSELF
        if (mounted) {

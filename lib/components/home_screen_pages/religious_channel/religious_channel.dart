...
// ✅ Reusing existing pages as requested (Ensure these can handle religious data)
...
class ProfessionalColors {
  ...
  static List<Color> gradientColors = [
    accentOrange, // Changed priority for Religious theme
    accentRed,
    accentPurple,
    accentBlue,
    accentGreen,
...
// ✅ ==========================================================
// ✅ [RENAMED] Religious Network Model
// ✅ ==========================================================
...
// ✅ ==========================================================
// ✅ [RENAMED & MODIFIED] ReligiousNetworkService (NO CACHE)
// ✅ ==========================================================
class ReligiousNetworkService {
  
  /// Main method to get all Religious Networks (Direct API Call)
  static Future<List<ReligiousNetworkModel>> getAllReligiousNetworks() async {
    try {
      print('🌐 Loading Fresh Religious Networks from API...'); 
    ...
    try {
      ...
      final response = await https
          .post(url,
            ...
            // ✅ [IMPORTANT] Changed "data_for" to "religious"
      ...
      if (response.statusCode == 200) {
        ...
        final allNetworks = jsonData
            .map((json) =>
                ReligiousNetworkModel.fromJson(json as Map<String, dynamic>))
            .toList();
        ...
        print(
            '✅ Successfully loaded ${activeNetworks.length} active Religious networks');
      ...
      print('❌ Error fetching religious networks: $e');
      rethrow;
    }
  }
}
...
// ✅ ==========================================================
// ✅ [RENAMED] Main Widget: ManageReligiousShows
// ✅ ==========================================================
class ManageReligiousShows extends StatefulWidget {
  const ManageReligiousShows({super.key});
  @override
...
class _ManageReligiousShowsState extends State<ManageReligiousShows>
  ...
  void initState() {
    ...
    fetchReligiousNetworks(); // ✅ Direct Fetch
  }
  ...
  void dispose() {
    ...
    if (_fullReligiousList.isNotEmpty) {
      firstNetworkId = _fullReligiousList[0].id.toString();
    }
  ...
  void _onViewAllFocusChange() {
    if (mounted && _viewAllFocusNode.hasFocus) {
      setState(() {
        focusedIndex = _displayedReligiousList.length; 
      });
  ...
  void _scrollToPosition(int index) {
    ...
    try {
      ...
      print('Error scrolling in religious list: $e');
    }
  }
  ...
  void _setupReligiousFocusProvider() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _displayedReligiousList.isNotEmpty) {
        try {
          final focusProvider =
              Provider.of<FocusProvider>(context, listen: false);
          final firstNetworkId = _displayedReligiousList[0].id.toString();
          _firstReligiousFocusNode =
              religiousFocusNodes[firstNetworkId];

          if (_firstReligiousFocusNode != null) {
            // ✅ [UPDATED KEY] 'religiousShows' to avoid conflict with 'tvShows'
            focusProvider.registerFocusNode(
                'religiousChannels', _firstReligiousFocusNode!);
          ...
          print('❌ Religious focus provider setup failed: $e');
        }
      }
    });
  }
  ...
  // ✅ [RENAMED] Navigation
  void _navigateToReligiousDetails(ReligiousNetworkModel network) async {
    print('🎬 Navigating to Religious Details: ${network.name}');

    try {
      int? currentUserId = SessionManager.userId;
      await HistoryService.updateUserHistory(
        userId: currentUserId!,
        contentType: 4, // Assuming 4 covers channels/networks
        eventId: network.id,
        eventTitle: network.name,
        url: '',
        categoryId: 0, // You might need a specific category ID for Religious if applicable
      );
    } catch (e) {
  ...
  void _navigateToGridPageWithNetwork(ReligiousNetworkModel network) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ReligiousChannelSliderScreen (
  ...
  Widget _buildReligiousItem(ReligiousNetworkModel network, int index,
      double screenWidth, double screenHeight) {
      ...
      onFocusChange: (hasFocus) async {
        ...
        if (hasFocus) {
          ...
          bool isAnyItemFocused =
              religiousFocusNodes.values.any((node) => node.hasFocus);
          if (!mounted) return;
          ...
          if (key == LogicalKeyboardKey.arrowRight) {
            if (index < _displayedReligiousList.length - 1) {
              String nextNetworkId =
                  _displayedReligiousList[index + 1].id.toString();
              FocusScope.of(context)
                  .requestFocus(religiousFocusNodes[nextNetworkId]);
            ...
            if (index > 0) {
              String prevNetworkId =
                  _displayedReligiousList[index - 1].id.toString();
              FocusScope.of(context)
                  .requestFocus(religiousFocusNodes[prevNetworkId]);
            }
            return KeyEventResult.handled;
          } else if (key == LogicalKeyboardKey.arrowUp) {
            setState(() {
        ...
        if (event is RawKeyDownEvent) {
          ...
          if (key == LogicalKeyboardKey.arrowLeft) {
            if (_displayedReligiousList.isNotEmpty) {
              String prevNetworkId =
                  _displayedReligiousList.last.id.toString();
              FocusScope.of(context)
                  .requestFocus(religiousFocusNodes[prevNetworkId]);
            }
            return KeyEventResult.handled;
  ...
  Widget _buildBody(double screenWidth, double screenHeight) {
    if (isLoading) {
      return ProfessionalReligiousLoadingIndicator(
          message: 'Loading Religious Channels...');
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(
              Icons.self_improvement, // ✅ Icon Updated for Religious theme
              size: 40,
              color: ProfessionalColors.accentOrange,
            ),
          ),
          const SizedBox(height: 24),
          const Text(
            'No Religious Channels Found', // ✅ Text Updated
            style: TextStyle(
              color: ProfessionalColors.textPrimary,
              fontSize: 18,
              fontWeight: FontWeight.w600,

...
//   //     if (mounted) {
//   //       final focusProvider = Provider.of<FocusProvider>(context, listen: false);
//   //       const String myId = 'religiousChannels'; // Is page ka identifier
...
//   //         if (_firstReligiousFocusNode != null) {
//   //           // 1. Register karein
//   //           focusProvider.registerFocusNode(myId, _firstReligiousFocusNode!);
            
//   //           // 2. ✅ CRITICAL FIX: Agar Dashboard focus maang raha hai
//   //           if (focusProvider.lastFocusedIdentifier == myId) {
//   //             _firstReligiousFocusNode!.requestFocus();
//   //           }
...
//             // 2. ✅ CRITICAL FIX: Check karein agar dashboard ne focus maanga hai
//             if (focusProvider.lastFocusedIdentifier == myId) {
//               // Thoda delay taaki UI build ho sake
//               Future.delayed(const Duration(milliseconds: 100), () {
//                 if (mounted && _firstReligiousFocusNode!.canRequestFocus) {
//                   _firstReligiousFocusNode!.requestFocus();
//                 }
//               });
//             }
...
//           if (focusProvider.lastFocusedIdentifier == myId) {
//              Future.delayed(const Duration(milliseconds: 100), () {
//                 if (mounted && _retryFocusNode.canRequestFocus) {
//                   _retryFocusNode.requestFocus();
//                 }
//              });
//           }
...
//           // _buildReligiousItem ke andar onKey handler mein:
// // ✅ 3. DOWN ARROW UPDATE (Next Page ke liye)
// else if (key == LogicalKeyboardKey.arrowDown) {
//   setState(() { focusedIndex = -1; _hasReceivedFocus = false; });
//   context.read<ColorProvider>().resetColor();
...
//   // Agle page ka identifier (e.g. 'tvshowpak' ya jo bhi iske baad aata ho) set karein
//   fp.updateLastFocusedIdentifier('tvshowpak'); 
...
//   // Agle page ka identifier (e.g. 'tvshowpak') set karein
//   fp.updateLastFocusedIdentifier('tvshowpak');
...
class ManageReligiousShows extends StatefulWidget {
  const ManageReligiousShows({super.key});
  @override
  State<ManageReligiousShows> createState() => _ManageReligiousShowsState();
}

class _ManageReligiousShowsState extends State<ManageReligiousShows> with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;

  Future<List<CommonContentModel>> fetchReligiousAPI() async {
    var url = Uri.parse(SessionManager.baseUrl + 'getNetworks');
    ...
    } else { throw Exception('Failed to load religious channels'); }
  }
  ...
  Future<void> _onItemTap(CommonContentModel item) async {
    ...
    await Navigator.push(context, MaterialPageRoute(builder: (context) => ReligiousChannelSliderScreen(initialNetworkId: int.parse(item.id))));
  }

  Future<void> _onViewAllTap() async {
    await Navigator.push(context, MaterialPageRoute(builder: (context) => const ReligiousChannelSliderScreen(initialNetworkId: null)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SmartCommonHorizontalList(
      ...
      titleGradient: const [ProfessionalColorsForHomePages.accentOrange, ProfessionalColorsForHomePages.accentRed],
      accentColor: ProfessionalColorsForHomePages.accentOrange,
      placeholderIcon: Icons.self_improvement, badgeDefaultText: 'RELIGIOUS',
      focusIdentifier: 'religiousChannels',
      fetchApiData: fetchReligiousAPI,

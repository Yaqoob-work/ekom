...
class _ReligiousChannelDetailsPageState
  ...
  Future<void> _fetchShowsFromAPI({bool showLoading = false}) async {
    ...
    try {
      ...
      if (shows != null) {
        final activeShows = _filterActiveShows(shows);
        ...
        if (_filteredShows.isNotEmpty) {
          _pageTransitionController.forward();
          // Automatically fetch episodes for the first show
          _fetchEpisodes(_filteredShows[0].id);
  ...
  Future<List<ReligiousShowModel>?> _fetchShowsFromAPIDirectly() async {
    String authKey = SessionManager.authKey;
    var url =
        Uri.parse(SessionManager.baseUrl + 'getReligiousShows/${widget.id}');
    ...
    throw Exception(
        'Failed to load religious shows (${response.statusCode})');
  }
  ...
  Future<List<ReligiousEpisodeModel>?> _fetchEpisodesFromAPIDirectly(
    ...
    String authKey = SessionManager.authKey;
    var url = Uri.parse(
        SessionManager.baseUrl + 'getReligiousShowsEpisodes/$showId');
  ...
  Future<void> _refreshDataOnReturn() async {
    // Refresh current show's episodes to get latest data
    if (_filteredShows.isNotEmpty &&
        _selectedShowIndex < _filteredShows.length) {
      final currentShowId = _filteredShows[_selectedShowIndex].id;
      // Clear memory cache for this show to force reload
      _filteredEpisodesMap.remove(currentShowId);
      await _fetchEpisodes(currentShowId);
    }
  }
  ...
  Future<void> _playEpisode(ReligiousEpisodeModel episode) async {
    ...
    try {
      ...
      await HistoryService.updateUserHistory(
        ...
        contentType: 7, // Content Type for Religious Episodes
        eventId: parsedId!,
  ...
  Widget _buildShowItem(int index) {
    ...
    final String uniqueImageUrl = "${show.thumbnail}?v=${show.updatedAt}";
    final String uniqueCacheKey =
        "${show.id.toString()}_${show.updatedAt}";
  ...
  void _onEpisodeTap(int index) {
    if (_currentEpisodes.isNotEmpty && index < _currentEpisodes.length) {
      ...
      _episodeFocusNodes[_currentEpisodes[index].id.toString()]
          ?.requestFocus();
      _playEpisode(_currentEpisodes[index]);
    }
  }

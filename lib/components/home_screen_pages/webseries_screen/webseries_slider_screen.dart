...
//   @override
//   _WebseriesSliderScreenState createState() =>
//       _WebseriesSliderScreenState();
// }
...
//       var url = Uri.parse(SessionManager.baseUrl + 'getAllWebSeries');
...
// import 'dart:convert';
// import 'dart:ui';
// // import 'package:cached_network_image/cached_network_image.dart'; // Removed as requested
// import 'package:flutter/material.dart';
// import 'package:flutter/services.dart';
// import 'dart:math' as math;
// import 'package:http/http.dart' as https;
...
// // NOTE: Apne project ke anusaar imports check karein
// import 'package:mobi_tv_entertainment/components/home_screen_pages/webseries_screen/webseries_details_page.dart';
// import 'package:mobi_tv_entertainment/main.dart';
// import 'package:mobi_tv_entertainment/components/services/history_service.dart';
...
//   List<ApiNetworkModel> _apiNetworks = [];
//   List<String> _uniqueNetworks = [];
//   List<String> _uniqueGenres = []; // Populated via API
...
//   // Helper function to fetch Genres from API
...
//     List<String> newGenres = await _fetchGenresFromApi(selectedNetworkId);

//     if (!mounted) return;

//     setState(() {
//       _uniqueGenres = newGenres;
...
//         _currentWebSeriesSliders.isNotEmpty) {
//       _sliderPageController.jumpToPage(0);
//     }
//   }
...
//   Future<List<WebSeriesModel>> _performSearchInNetwork(
...
//     return networkSeries
//         .where((series) =>
//             series.name.toLowerCase().contains(searchTerm.toLowerCase()))
//         .toList();
//   }
...
//       }
//       if (key == LogicalKeyboardKey.arrowRight && _genreFocusNodes.isNotEmpty) {
//         // If there are genres (even loading placeholder), move to first genre
//         if (_genreFocusNodes.isNotEmpty) {
//            _genreFocusNodes[0].requestFocus();
//         }
//         return KeyEventResult.handled;
...
//     if (key == LogicalKeyboardKey.arrowUp) {
//       setState(() => _lastNavigationDirection = 'vertical');
//       if (_genreFocusNodes.isNotEmpty && !_isGenreLoading) {
//         _genreFocusNodes[_focusedGenreIndex].requestFocus();
//       } else {
//         _searchButtonFocusNode.requestFocus();
//       }
...
//           currentList[_focusedItemIndex], _focusedItemIndex);
//       return;
//     }
...
//   void _navigateGenres(LogicalKeyboardKey key) {
//     if (_isGenreLoading) return; // Loading ke waqt genre navigation disable
...
//         ),
//         _buildSliderIndicators(),
//         _buildGenreAndSearchButtons(), // UPDATED FUNCTION
//         SizedBox(height: screenhgt * 0.02),
//         _buildWebSeriesList(),
//       ],
//     );
...
//   // FIX: This widget now keeps Search Button visible even during loading
//   Widget _buildGenreAndSearchButtons() {
//     // Determine total item count: 1 (Search) + (Loading ? 1 : Genre Count)
//     int itemCount = 1 + (_isGenreLoading ? 1 : _uniqueGenres.length);
...
//           padding: EdgeInsets.symmetric(horizontal: screenwdt * 0.015),
//           itemBuilder: (context, index) {
//             // --- 1. SEARCH BUTTON (Always Visible & First) ---
//             if (index == 0) {
//               return Focus(
//                 focusNode: _searchButtonFocusNode,
//                 child: _buildGlassEffectButton(
...
//             // --- 2. LOADING INDICATOR (If loading) ---
//             if (_isGenreLoading) {
//               return Container(
...
//                   if (hasFocus) {
//                     setState(() => _focusedItemIndex = index);
//                     // Added internal focus provider update
//                     Provider.of<InternalFocusProvider>(context, listen: false)
...
// //==============================================================================
// // SECTION 1: COMMON CLASSES AND MODELS (KEPT EXACTLY SAME)
// //==============================================================================
...
// //==============================================================================
// // SECTION 2: MAIN PAGE WIDGET AND STATE (CRASH FIXES APPLIED HERE)
// //==============================================================================
...
//   void initState() {
//     super.initState();
//     _isDisposed = false; // CRASH FIX: Init flag
//     _sliderPageController = PageController();
//     _searchButtonFocusNode = FocusNode();
//     _searchButtonFocusNode.addListener(() {
...
//   @override
//   void dispose() {
//     _isDisposed = true; // CRASH FIX: Set flag first
//     _sliderPageController.dispose();
//     _fadeController.dispose();
//     _widgetFocusNode.dispose();
//     _listScrollController.dispose();
...
//   Future<List<String>> _fetchGenresFromApi(int networkId) async {
//     if (_isDisposed) return ['All']; // CRASH FIX
//     try {
//       final authKey = SessionManager.authKey;
//       var url = Uri.parse(SessionManager.baseUrl + 'getGenreByContentNetwork');
...
//       if (_isDisposed) return ['All']; // CRASH FIX: Check again after await
...
//               try { _networkFocusNodes[0].requestFocus(); } catch(_){}
//             }
//           });
//         }
//         setState(() => _isLoading = false);
...
//   void _focusFirstListItemWithScroll() {
//     if (_itemFocusNodes.isEmpty || _isDisposed) return;
//     if (_listScrollController.hasClients) {
...
//   KeyEventResult _onKeyHandler(FocusNode node, RawKeyEvent event) {
//     if (_isDisposed || event is! RawKeyDownEvent) return KeyEventResult.ignored;
...
//         ),
//       ),
//     ).catchError((e) => null); // CRASH FIX: Catch nav errors
...
//   void _initializeFocusNodes() {
//     _disposeFocusNodes(_networkFocusNodes);
//     _networkFocusNodes = List.generate(_uniqueNetworks.length, (index) => FocusNode(debugLabel: 'Network-$index'));
//     _rebuildGenreFocusNodes();
//     _rebuildItemFocusNodes();
//     _rebuildKeyboardFocusNodes();
//   }
...
//   void _rebuildGenreFocusNodes() {
//     _disposeFocusNodes(_genreFocusNodes);
//     _genreFocusNodes = List.generate(_uniqueGenres.length, (index) => FocusNode(debugLabel: 'Genre-$index'));
//   }

//   void _rebuildItemFocusNodes() {
//     _disposeFocusNodes(_itemFocusNodes);
//     final currentList = _isSearching ? _searchResults : _filteredWebSeriesList;
//     _itemFocusNodes = List.generate(currentList.length, (index) => FocusNode(debugLabel: 'Item-$index'));
//   }
...
//   //=================================================
//   // WIDGET BUILDER METHODS (UI SAME AS PROVIDED)
//   //=================================================
...
//   // ... (Using same _buildSearchUI, _buildQwertyKeyboard, _buildSliderIndicators, _buildGlassEffectButton, _buildErrorWidget as your code)
//   // Re-pasting helper widgets below for full file completeness
...
//         final screenWidth = MediaQuery.of(context).size.width;
//         final screenHeight = MediaQuery.of(context).size.height;
//         double width = (key == ' ') ? screenWidth * 0.315 : (key == 'OK' || key == 'DEL' ? screenWidth * 0.09 : screenWidth * 0.045);
...
//               onPressed: () => _onKeyPressed(key),
//               style: ElevatedButton.styleFrom(
//                 backgroundColor: isFocused ? ProfessionalColors.accentPurple : Colors.white.withOpacity(0.1),
//                 foregroundColor: Colors.white,
//                 shape: RoundedRectangleBorder(
//                   borderRadius: BorderRadius.circular(12.0),
//                   side: isFocused ? const BorderSide(color: Colors.white, width: 3) : BorderSide.none,
//                 ),
//                 padding: EdgeInsets.zero,
//               ),
//               child: Text(key, style: TextStyle(fontSize: 22.0, fontWeight: isFocused ? FontWeight.bold : FontWeight.normal)),
//             ),
//           ),
//         );
//       }).toList(),
...
//   Widget _buildSliderIndicators() {
//     if (_currentWebSeriesSliders.length <= 1) return const SizedBox.shrink();
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: List.generate(_currentWebSeriesSliders.length, (index) {
//         bool isActive = _currentSliderIndex == index;
...
//   Widget _buildGlassEffectButton({
//     required FocusNode focusNode, required VoidCallback onTap, required bool isSelected,
//     required Color focusColor, required Widget child,
//   }) {
//     bool hasFocus = focusNode.hasFocus;
//     return GestureDetector(
//       onTap: onTap,
...
//               padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 1),
//               decoration: BoxDecoration(
//                 color: hasFocus ? focusColor : isSelected ? focusColor.withOpacity(0.5) : Colors.white.withOpacity(0.08),
//                 borderRadius: BorderRadius.circular(30),
//                 border: Border.all(color: hasFocus ? Colors.white : Colors.white.withOpacity(0.3), width: hasFocus ? 3 : 2),
//                 boxShadow: (hasFocus || isSelected) ? [BoxShadow(color: focusColor.withOpacity(0.8), blurRadius: 15, spreadRadius: 3)] : null,
//               ),
//               child: child,
//             ),
//           ),
...
//         decoration: BoxDecoration(color: ProfessionalColors.surfaceDark.withOpacity(0.5), borderRadius: BorderRadius.circular(24)),
//         child: Column(
//           mainAxisSize: MainAxisSize.min,
//           children: [
//             const Icon(Icons.cloud_off_rounded, color: Colors.red, size: 60),
//             const SizedBox(height: 24),
//             Text(_errorMessage ?? 'Something went wrong.', textAlign: TextAlign.center, style: const TextStyle(color: ProfessionalColors.textPrimary, fontSize: 18)),
//             const SizedBox(height: 24),
//             ElevatedButton.icon(
//               onPressed: () => _fetchDataForPage(forceRefresh: true),
//               icon: const Icon(Icons.refresh_rounded), label: const Text('Try Again'),
//               style: ElevatedButton.styleFrom(backgroundColor: ProfessionalColors.accentBlue),
//             ),
//           ],
//         ),
//       ),
...
// //==============================================================================
// // SECTION 3: REUSABLE UI COMPONENTS (KEPT EXACTLY SAME)
// //==============================================================================
...
//   const OptimizedWebSeriesCard({
//     Key? key, required this.webSeries, required this.isFocused, required this.onTap,
//     required this.cardHeight, this.networkLogo, required this.uniqueIndex,
//   }) : super(key: key);

//   @override
//   Widget build(BuildContext context) {
//     final focusColor = [
//       ProfessionalColors.accentBlue, ProfessionalColors.accentPurple,
//       ProfessionalColors.accentPink, ProfessionalColors.accentOrange,
//       ProfessionalColors.accentRed
//     ][uniqueIndex % 5];
...
//             decoration: BoxDecoration(
//               borderRadius: BorderRadius.circular(8.0),
//               border: Border.all(color: isFocused ? focusColor : Colors.transparent, width: 3),
//               boxShadow: isFocused ? [BoxShadow(color: focusColor.withOpacity(0.5), blurRadius: 12, spreadRadius: 1)] : [],
//             ),
//             child: ClipRRect(
//               borderRadius: BorderRadius.circular(6.0),
//               child: Stack(
//                 fit: StackFit.expand,
//                 children: [
//                   webSeries.banner != null ? Image.network(webSeries.banner!, fit: BoxFit.cover, errorBuilder: (c,e,s)=>Container(color: Colors.grey)) : Container(color: Colors.grey),
//                   if (isFocused) Center(child: Icon(Icons.play_circle_filled_outlined, color: focusColor, size: 40)),
//                   if (networkLogo != null) Positioned(top: 5, right: 5, child: CircleAvatar(radius: 12, backgroundImage: NetworkImage(networkLogo!))),
//                 ],
//               ),
//             ),
//           ),
...
//           child: Text(webSeries.name, style: TextStyle(color: isFocused ? focusColor : ProfessionalColors.textSecondary, fontWeight: isFocused ? FontWeight.bold : FontWeight.normal), maxLines: 1, overflow: TextOverflow.ellipsis),
//         ),
//       ],
//     );
//   }
...
//   final ValueChanged<int> onPageChanged;
//   final PageController controller;
//   const WebSeriesBannerSlider({Key? key, required this.sliders, required this.onPageChanged, required this.controller}) : super(key: key);
//   @override
//   _WebSeriesBannerSliderState createState() => _WebSeriesBannerSliderState();
// }
...
//       onPageChanged: widget.onPageChanged,
//       itemBuilder: (context, index) {
//         return Image.network(widget.sliders[index].banner, fit: BoxFit.cover, errorBuilder: (c,e,s)=>Container(color: ProfessionalColors.surfaceDark));
//       },
//     );
//   }
// }
...
// import 'package:mobi_tv_entertainment/components/services/history_service.dart';
// import 'package:mobi_tv_entertainment/components/provider/internal_focus_provider.dart';
// import 'package:mobi_tv_entertainment/components/widgets/smart_style_image_card.dart'; // 🔥 ADDED IMPORT
...
// //==============================================================================
// // SECTION 3: REUSABLE UI COMPONENTS (🔥 UPDATED TO USE SMART STYLE IMAGE CARD 🔥)
// //==============================================================================
...
//             fit: StackFit.expand,
//             children: [
//               webSeries.banner != null
//                   ? Image.network(
//                       webSeries.banner!,
//                       fit: BoxFit.cover,
//                       cacheWidth: (bannerwdt * 1.5).toInt(),
//                       errorBuilder: (c, e, s) => Container(color: ProfessionalColors.cardDark),
...
class _WebseriesSliderScreenState extends State<WebseriesSliderScreen> {
  bool _isDisposed = false;
  bool _isLoading = true;
  bool _isVideoLoading = false;
  String? _errorMessage;
  ...
  List<ApiNetworkModel> _apiNetworks = [];
  List<WebSeriesModel> _allWebSeries = [];
    ...
    try {
      ...
      if (results[0].statusCode == 200 && results[1].statusCode == 200) {
        ...
        if (_apiNetworks.isNotEmpty) {
          ...
          _genres = await _fetchGenresFromApi(_apiNetworks[0].id);
          _updateDisplayData();
        } else {

...
//   @override
//   _ ManageWebSeriesState createState() =>
//       _ ManageWebSeriesState();
// }

// class _ ManageWebSeriesState
//     extends State< ManageWebSeries>
...
// import 'package:mobi_tv_entertainment/home_screen_pages/webseries_screen/professional_web_series_grid_page.dart';
// import 'package:mobi_tv_entertainment/home_screen_pages/webseries_screen/webseries_details_page.dart';
// import 'package:mobi_tv_entertainment/main.dart';
// import 'package:mobi_tv_entertainment/provider/color_provider.dart';
// import 'package:mobi_tv_entertainment/provider/focus_provider.dart';
...
// class WebSeriesService {
//   // Cache keys for the full list (used by Grid Page)
//   static const String _cacheKeyWebSeries = 'cached_web_series';
//   static const String _cacheKeyTimestamp = 'cached_web_series_timestamp';
//   static const int _cacheDurationMs = 60 * 60 * 1000;

//   // Cache keys for the limited list (used by Horizontal List)
...
//   // /// Fetches ALL web series, used by the Grid Page.
//   // static Future<List<WebSeriesModel>> getAllWebSeries(
//   //     {bool forceRefresh = false}) async {
//   //   try {
//   //     final prefs = await SharedPreferences.getInstance();
//   //     if (!forceRefresh && await _shouldUseCache(prefs, isLimited: false)) {
//   //       final cachedWebSeries = await _getCachedWebSeries(prefs, isLimited: false);
//   //       if (cachedWebSeries.isNotEmpty) {
//   //         _loadFreshDataInBackground(isLimited: false);
//   //         return cachedWebSeries;
//   //       }
//   //     }
//   //     return await _fetchFreshWebSeries(prefs, isLimited: false);
//   //   } catch (e) {
//   //     print('❌ Error in getAllWebSeries: $e');
//   //     try {
//   //       final prefs = await SharedPreferences.getInstance();
...
//   /// Fetches a LIMITED number of web series, used by the Horizontal List.
//   static Future<List<WebSeriesModel>> getLimitedWebSeries(
//       {bool forceRefresh = false}) async {
//     try {
//       final prefs = await SharedPreferences.getInstance();
//       if (!forceRefresh && await _shouldUseCache(prefs, isLimited: true)) {
//         final cachedWebSeries = await _getCachedWebSeries(prefs, isLimited: true);
//         if (cachedWebSeries.isNotEmpty) {
//           _loadFreshDataInBackground(isLimited: true);
//           return cachedWebSeries;
//         }
//       }
//       return await _fetchFreshWebSeries(prefs, isLimited: true);
//     } catch (e) {
...
//   static Future<bool> _shouldUseCache(SharedPreferences prefs, {required bool isLimited}) async {
//     final key = isLimited ? _cacheKeyLimitedTimestamp : _cacheKeyTimestamp;
...
//   static Future<List<WebSeriesModel>> _getCachedWebSeries(
//       SharedPreferences prefs, {required bool isLimited}) async {
//     final key = isLimited ? _cacheKeyLimitedWebSeries : _cacheKeyWebSeries;
//     try {
...
//       return webSeries;
//     } catch (e) {
//       print('❌ Error loading cached web series (isLimited: $isLimited): $e');
//       return [];
//     }
//   }
...
//       }
//     } catch (e) {
//       print('❌ Error fetching fresh web series (isLimited: $isLimited): $e');
//       rethrow;
//     }
//   }

//   static Future<void> _cacheWebSeries(
//       SharedPreferences prefs, List<dynamic> webSeriesData, {required bool isLimited}) async {
//     final cacheKey = isLimited ? _cacheKeyLimitedWebSeries : _cacheKeyWebSeries;
//     final timestampKey = isLimited ? _cacheKeyLimitedTimestamp : _cacheKeyTimestamp;
//     try {
//       final jsonString = json.encode(webSeriesData);
//       final currentTimestamp = DateTime.now().millisecondsSinceEpoch.toString();
//       await Future.wait([
...
//       ]);
//     } catch (e) {
//       print('❌ Error caching web series (isLimited: $isLimited): $e');
//     }
//   }

//   static void _loadFreshDataInBackground({required bool isLimited}) {
//     Future.delayed(const Duration(milliseconds: 500), () async {
//       try {
//         final prefs = await SharedPreferences.getInstance();
//         await _fetchFreshWebSeries(prefs, isLimited: isLimited);
//       } catch (e) {
//         print('⚠️ WebSeries background refresh failed (isLimited: $isLimited): $e');
//       }
//     });
//   }
// }
...
//   const  ManageWebSeries({super.key});
//   @override
//   _ManageWebSeriesState createState() =>
//       _ManageWebSeriesState();
// }

// class _ManageWebSeriesState
//     extends State< ManageWebSeries>
//     with AutomaticKeepAliveClientMixin, TickerProviderStateMixin {
//   @override
//   bool get wantKeepAlive => true;
...
//           if (event.logicalKey == LogicalKeyboardKey.arrowLeft) {
//             FocusScope.of(context).requestFocus(
//                 webseriesFocusNodes[webSeriesList[maxHorizontalItems - 1].id.toString()]);
//             return KeyEventResult.handled;
//           } else if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
//             return KeyEventResult.handled;
//           } else if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
...
//   // ✅ MODIFIED: Throttling logic state variables
...
//   @override
//   void dispose() {
//     // ✅ MODIFIED: Cancel timer to prevent memory leaks
...
//         }
//       },
//     // ✅ MODIFIED: The onKey handler is replaced with the new, more robust throttling logic.
//       onKey: (node, event) {
//         if (event is RawKeyDownEvent) {

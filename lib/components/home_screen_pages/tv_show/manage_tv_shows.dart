...
// // 🚀 Enhanced  ManageTvShows with Caching (WebSeries Style)
// class  ManageTvShows extends StatefulWidget {
//   const  ManageTvShows({super.key});
//   @override
//   _ManageTvShowsState createState() =>
//       _ManageTvShowsState();
// }

// class _ManageTvShowsState
//     extends State< ManageTvShows>
...
//     try {
//       double bannerwidth = bannerwdt; // Assuming bannerwdt is available.
...
//     } catch (e) {
//       // Silent fail
//       print('Error scrolling in tv shows: $e');
//     }
//   }
...
// import 'dart:math' as math;
// // import 'package:mobi_tv_entertainment/home_screen_pages/tv_show/tv_show_second_page.dart'; // ✅ REMOVED: Grid page import
// import 'package:mobi_tv_entertainment/main.dart';
...
//   void _initializeFocusNodes() {
//     // No need to initialize view all focus node anymore
//     print('✅ TV Shows focus nodes initialized');
//   }
...
//     try {
//       double bannerwidth = bannerwdt + 5; // Assuming bannerwdt is available.
...
//           focusProvider.registerFocusNode('tvShows', _firstTVShowFocusNode!);
//           print(
//               '✅ TV Shows first focus node registered: ${tvShowsList[0].name}');
//         } catch (e) {
//           print('❌ TV Shows focus provider setup failed: $e');
...
//     // ✅ REMOVED: Loop limit, now creates nodes for all items
...
//           int currentIndex =
//               tvShowsList.indexWhere((show) => show.id == tvShow.id);
//           // ✅ REMOVED: Limit on restoring focus
//           if (currentIndex != -1) {
//             String tvShowId = tvShow.id.toString();
//             if (tvshowsFocusNodes.containsKey(tvShowId)) {
//               setState(() {
...
//   // ✅ REMOVED: _navigateToGridPage method
...
//       // onKey: (FocusNode node, RawKeyEvent event) {
//       //   if (event is RawKeyDownEvent) {
//       //     // ✅ UPDATED: Simplified right arrow navigation
//       //     if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
//       //       if (index < tvShowsList.length - 1) {
//       //         String nextTVShowId = tvShowsList[index + 1].id.toString();
//       //         FocusScope.of(context)
//       //             .requestFocus(tvshowsFocusNodes[nextTVShowId]);
//       //         return KeyEventResult.handled;
//       //       }
//       //       // ✅ REMOVED: Logic to focus ViewAll button
//       //     } else if (event.logicalKey == LogicalKeyboardKey.arrowLeft) {
//       //       if (index > 0) {
//       //         String prevTVShowId = tvShowsList[index - 1].id.toString();
//       //         FocusScope.of(context)
...
//           padding: EdgeInsets.symmetric(horizontal: screenWidth * 0.025),
//           cacheExtent: 1200,
//           // ✅ UPDATED: Item count to show all TV shows
//           itemCount: tvShowsList.length,
//           itemBuilder: (context, index) {
//             // ✅ REMOVED: ViewAll button logic
//             var tvShow = tvShowsList[index];
//             return _buildTVShowItem(tvShow, index, screenWidth, screenHeight);
//           },
//         ),
...
//             Row(
//               children: [
//                 // Refresh button and count can be added here if needed
//               ],
//             ),
//           ],
//         ),
...
// //     // ✅ REMOVED: Disposal of _viewAllFocusNode
// //     try {
// //       _scrollController.dispose();
// //     } catch (e) {}
...
// // ✅ REMOVED: ProfessionalTVShowViewAllButton widget
...
// // ✅ REMOVED: ProfessionalTVShowsGridPage widget
// // ✅ REMOVED: ProfessionalGridTVShowCard widget
...
//   //           // Node ko provider mein 'tvShows' ID se register karein
//   //           focusProvider.registerFocusNode('tvShows', _firstTVShowFocusNode!);
...
//                 // _hasReceivedFocusFromWebSeries flag ko sirf tab set karein jab 
//                 // focus pehli baar row mein aa raha hai.
//                 if (!_hasReceivedFocusFromWebSeries) {
//                   _hasReceivedFocusFromWebSeries = true;
//                 }
...
//                 _scrollToPosition(0);
//                 print('✅ TV Shows (first item) received focus and scrolled');
//               }
//               // ✅ ==========================================================
...
//     for (int i = 0; i < tvShowsList.length; i++) {
//       String tvShowId = tvShowsList[i].id.toString();
//       tvshowsFocusNodes[tvShowId] = FocusNode(); // Naya node banayein
...
//         eventId: parsedId!, // 3. Event ID (channel की ID)
//         eventTitle: tvShow.name, // 4. Event Title (channel का नाम)
//         url: '', // 5. URL (channel का URL) - Agar hai toh
//         categoryId: 0, // 6. Category ID (डिफ़ॉल्ट 0)
//       );
//     } catch (e) {
//       print("History update failed, but proceeding to play. Error: $e");
//     }
...
//       TVShowModel tvShow, int index, double screenWidth, double screenHeight) {
//     String tvShowId = tvShow.id.toString();
//     FocusNode? focusNode = tvshowsFocusNodes[tvShowId];
...
//             setState(() {
//               _currentAccentColor = dominantColor;
//               focusedIndex = index; // Update focused index
//               _hasReceivedFocusFromWebSeries = true; // Mark as focused
//             });
//             context.read<ColorProvider>().updateColor(dominantColor, true);
...
//       await Future.wait([
//         TVShowsService.clearCache(),
//         // WebSeriesService.clearCache(), // Uncomment if you add this service
//         // MoviesService.clearCache(), // Uncomment if you add this service
//       ]);
//       print('🗑️ All caches cleared successfully');
//     } catch (e) {
//       print('❌ Error clearing all caches: $e');
...
//     try {
//       final tvShowsCacheInfo = await TVShowsService.getCacheInfo();
//       // final webSeriesCacheInfo = await WebSeriesService.getCacheInfo(); // Uncomment
//       // final moviesCacheInfo = await MoviesService.getCacheInfo(); // Uncomment

//       return {
//         'tvShows': tvShowsCacheInfo,
//         // 'webSeries': webSeriesCacheInfo, // Uncomment
//         // 'movies': moviesCacheInfo, // Uncomment
//         'totalCacheSize': _calculateTotalCacheSize([
//           tvShowsCacheInfo,
//           // webSeriesCacheInfo, // Uncomment
//           // moviesCacheInfo, // Uncomment
//         ]),
//       };
//     } catch (e) {
//       print('❌ Error getting all cache info: $e');
...
//         'error': e.toString(),
//         'tvShows': {'hasCachedData': false},
//         // 'webSeries': {'hasCachedData': false}, // Uncomment
//         // 'movies': {'hasCachedData': false}, // Uncomment
//       };
//     }
//   }
...
//     int totalSize = 0;
//     for (final info in cacheInfos) {
//        // Check if 'cacheSize' exists and is an int
...
//       await Future.wait([
//         TVShowsService.forceRefresh(),
//         // WebSeriesService.forceRefresh(), // Uncomment
//         // MoviesService.forceRefresh(), // Uncomment
//       ]);
//       print('🔄 All data force refreshed successfully');
//     } catch (e) {
//       print('❌ Error force refreshing all data: $e');
...
// 🎯 YEH AAPKI NAYI/REPLACEMENT FILE HAI:
// components/home_screen_pages/tv_show/manage_tv_shows.dart
...
class TVShowNetworkService {
  ...
  static Future<List<TVShowNetworkModel>> getAllTVShowNetworks(
    ...
    try {
      ...
      try {
        ...
        final cachedNetworks = await _getCachedTVShowNetworks(prefs);
        if (cachedNetworks.isNotEmpty) {
          print('🔄 Returning cached TV show networks as fallback'); // ✅ UPDATED Text
          return cachedNetworks;
        }
      } catch (cacheError) {
        print('❌ Cache fallback also failed: $cacheError');
  ...
  static Future<bool> _shouldUseCache(SharedPreferences prefs) async {
    try {
      ...
      print('❌ Error checking TV Show Networks cache validity: $e'); // ✅ UPDATED Text
      return false;
    }
  }
  ...
  static Future<void> _cacheTVShowNetworks(
    ...
    try {
      ...
      print('💾 Successfully cached ${networkData.length} TV show networks'); // ✅ UPDATED Text
    } catch (e) {
  ...
  static void _loadFreshDataInBackground() {
    Future.delayed(const Duration(milliseconds: 500), () async {
      try {
        print('🔄 Loading fresh TV show networks data in background...'); // ✅ UPDATED Text
        final prefs = await SharedPreferences.getInstance();
        await _fetchFreshTVShowNetworks(prefs);
        print('✅ TV show networks background refresh completed'); // ✅ UPDATED Text
      } catch (e) {
        print('⚠️ TV show networks background refresh failed: $e'); // ✅ UPDATED Text
      }
    });
  }
  ...
  static Future<void> clearCache() async {
    try {
      ...
      print('🗑️ TV show networks cache cleared successfully'); // ✅ UPDATED Text
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> getCacheInfo() async {
    try {
      ...
      print('❌ Error getting TV show networks cache info: $e'); // ✅ UPDATED Text
...
class _ManageTvShowsState extends State<ManageTvShows>
  ...
  void dispose() {
    ...
    for (var entry in tvShowNetworkFocusNodes.entries) {
      if (entry.key !=
          firstNetworkId) { // Provider mein register node ko chhodkar
        try {
          entry.value.removeListener(() {});
          entry.value.dispose();
        } catch (e) {}
  ...
  Future<void> fetchTVShowNetworksWithCache() async {
    ...
    try {
      final fetchedNetworks =
          await TVShowNetworkService.getAllTVShowNetworks();

      if (mounted) {
        _fullTVShowNetworkList = fetchedNetworks;
        // ✅ [NEW] View All Logic
        if (_fullTVShowNetworkList.length > 10) {
          _displayedTVShowNetworkList =
              _fullTVShowNetworkList.sublist(0, 10);
        } else {
  ...
  Future<void> _debugCacheInfo() async {
    try {
      final cacheInfo = await TVShowNetworkService.getCacheInfo(); // ✅ [RENAMED]
  ...
  void _createFocusNodesForItems() {
    ...
    print(
        '✅ Created ${tvShowNetworkFocusNodes.length} network focus nodes');
  }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => TvShowSliderScreen(
          // networks: _fullTVShowNetworkList, // Pass the *full* list
        ),
      ),
    ).then((_) {
      print('🔙 Returned from Grid Page');
  ...
  Widget _buildTVShowNetworkItem(TVShowNetworkModel network, int index,
    ...
    return Focus(
        ...
        if (event is RawKeyDownEvent) {
          ...
          if (key == LogicalKeyboardKey.arrowRight) {
            ...
            FocusScope.of(context).unfocus(); // Unfocus current node first
             Future.delayed(const Duration(milliseconds: 50), () { // Short delay
                if (mounted) {
                   try {
            ...
            return KeyEventResult.handled;
              ...
              key == LogicalKeyboardKey.select) {
            _navigateToTVShowNetworkDetails(network);
            return KeyEventResult.handled;
          }
        }
        return KeyEventResult.ignored;
      ...
      child: GestureDetector(
        onTap: () => _navigateToTVShowNetworkDetails(network),
        child: ProfessionalTVShowNetworkCard(
          network: network,
          focusNode: focusNode,
          onTap: () => _navigateToTVShowNetworkDetails(network),

...
//       if (mounted) {
//         final focusProvider = Provider.of<FocusProvider>(context, listen: false);
//         const String myId = 'kids_show'; // Is page ka identifier
...
//           if (_firstKidsFocusNode != null) {
//             // 1. Register karein
//             focusProvider.registerFocusNode(myId, _firstKidsFocusNode!);
            
//             // 2. ✅ CRITICAL FIX: Agar Dashboard is page se focus maang raha hai
//             if (focusProvider.lastFocusedIdentifier == myId) {
//               _firstKidsFocusNode!.requestFocus();
//             }
...
class ManageKidsShows extends StatefulWidget {
  const ManageKidsShows({super.key});
  @override
  State<ManageKidsShows> createState() => _ManageKidsShowsState();
}

class _ManageKidsShowsState extends State<ManageKidsShows> with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;

  Future<List<CommonContentModel>> fetchKidsAPI() async {
    var url = Uri.parse(SessionManager.baseUrl + 'getNetworks');
    ...
    } else { throw Exception('Failed to load kids channels'); }
  }
  ...
  Future<void> _onItemTap(CommonContentModel item) async {
    ...
    await Navigator.push(context, MaterialPageRoute(builder: (context) => KidChannelsSliderScreen(initialNetworkId: int.parse(item.id))));
  }

  Future<void> _onViewAllTap() async {
    await Navigator.push(context, MaterialPageRoute(builder: (context) => const KidChannelsSliderScreen(initialNetworkId: null)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SmartCommonHorizontalList(
      sectionTitle: "KIDS ZONE",
      titleGradient: const [ProfessionalColorsForHomePages.accentPink, ProfessionalColorsForHomePages.accentBlue],
      accentColor: ProfessionalColorsForHomePages.accentPink,
      placeholderIcon: Icons.toys_rounded, badgeDefaultText: 'KIDS',
      focusIdentifier: 'kids_show',
      fetchApiData: fetchKidsAPI,

...
// class LiveKidsScreen extends StatefulWidget {
//   const LiveKidsScreen({super.key});
//   @override
//   State<LiveKidsScreen> createState() => _LiveKidsScreenState();
// }

// class _LiveKidsScreenState extends State<LiveKidsScreen> with AutomaticKeepAliveClientMixin {
//   @override
...
class _LiveKidsScreenState extends State<LiveKidsScreen> with AutomaticKeepAliveClientMixin {
  ...
  List<CommonContentModel> _allKidsList = [];

  Future<List<CommonContentModel>> fetchKidsAPI() async {
    var url = Uri.parse(SessionManager.baseUrl + 'getAllKidsLive');
    final response = await https.get(url, headers: {
      'auth-key': SessionManager.authKey, 
      'Content-Type': 'application/json', 
      'domain': SessionManager.savedDomain
    ...
    if (response.statusCode == 200) {
      ...
      _allKidsList = mappedList;
      return mappedList;

    } else { 
      throw Exception('Failed to load live kids'); 
    }
  }
  ...
  Future<void> _onItemTap(CommonContentModel item) async {
    ...
    // 🔥 3. _allKidsList ko NewsItemModel mein convert karke ek swipeable list banayein
    List<NewsItemModel> playerList = _allKidsList.map((c) {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return SmartCommonHorizontalList(
      sectionTitle: "LIVE KIDS",
      titleGradient: const [ProfessionalColorsForHomePages.accentBlue, ProfessionalColorsForHomePages.accentPurple],
      accentColor: ProfessionalColorsForHomePages.accentBlue,
      placeholderIcon: Icons.sports_cricket, 
      badgeDefaultText: 'LIVE',
      focusIdentifier: 'liveKids',

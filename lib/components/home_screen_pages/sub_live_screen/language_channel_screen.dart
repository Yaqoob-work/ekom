...
class _LanguageChannelsScreenState extends State<LanguageChannelsScreen> {
  ...
  Future<void> _fetchAndProcessData() async {
    ...
    try {
      ...
      var genreUrl = Uri.parse(SessionManager.baseUrl + 'getGenreByContentNetwork'); 

      // 2. Parallel Calls (Sliders, Channels, Genres)
      final results = await Future.wait([
        https.get(langUrl, headers: headers), // 0: Languages (Sliders)
        https.post(tvUrl, headers: headers, body: json.encode({"genere": "", "languageId": widget.languageId})), // 1: Channels
        https.post(genreUrl, headers: headers, body: json.encode({"language_id": widget.languageId})), // 2: Genres (NEW)
      ]);
      ...
      if (langRes.statusCode == 200) {
        try {
          ...
          if (languagesData['status'] == true && languagesData['languages'] is List) {
            final allLanguages = languagesData['languages'] as List;
            final currentLanguage = allLanguages.firstWhere(
              (lang) => lang['id'].toString() == widget.languageId,
              orElse: () => null,
            ...
            if (currentLanguage != null &&
              ...
              if (mounted) {
                setState(() {
                  _sliders = sliderData.map((item) => SliderItem.fromJson(item)).toList();
                });
              }
            }
          }
      ...
      if (tvRes.statusCode == 200 && genreRes.statusCode == 200) {
        ...
        if (apiGenres.isEmpty) {
           ...
           for (final channel in _allChannels) {
             final gList = channel.genres.split(',').map((g) => g.trim()).toList();
        ...
        // Distribute channels
        for (final channel in _allChannels) {
          final channelGenres = channel.genres.split(',').map((g) => g.trim()).toList();
        ...
        throw Exception('API Error. TV: ${tvRes.statusCode}, Genre: ${genreRes.statusCode}');
      }
    } catch (e) {
      if (mounted) {
        setState(() {

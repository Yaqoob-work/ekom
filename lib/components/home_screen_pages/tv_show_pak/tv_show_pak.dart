...
// import 'package:mobi_tv_entertainment/components/home_screen_pages/tv_show_pak/tv_show_second_page.dart';
...
//           focusProvider.registerFocusNode('tvShowsPak', _firstTVShowFocusNode!);
//           print(
...
//           // 3. अब फोकस बदलें
//           if (key == LogicalKeyboardKey.arrowRight) {
//             if (index < tvShowsList.length - 1 && index < maxHorizontalItems - 1) {
//               String nextTVShowId = tvShowsList[index + 1].id.toString();
//               FocusScope.of(context)
//                   .requestFocus(tvshowsPakFocusNodes[nextTVShowId]);
//             } else if (index == maxHorizontalItems - 1 && tvShowsList.length > maxHorizontalItems) {
//               FocusScope.of(context).requestFocus(_viewAllFocusNode);
//             } else {
...
//             if (mounted) {
//               try {
// Provider.of<FocusProvider>(context, listen: false).requestFocus('religiousChannels');
//               } catch (e) {
//                 print('❌ Failed to navigate to religious channels: $e');
//               }
//             }
//           });
//           return KeyEventResult.handled;
...
//     String? firstTVShowId;
//     if (tvShowsList.isNotEmpty) {
//       firstTVShowId = tvShowsList[0].id.toString();
//     }

//     for (var entry in tvshowsPakFocusNodes.entries) {
//       // Agar node register nahi hua hai (yaani first TV show nahi hai), tabhi use yahan dispose karein
//       if (entry.key != firstTVShowId) {
//         try {
//           entry.value.removeListener(() {});
//           entry.value.dispose();
//         } catch (e) {}
...
// import 'package:http/http.dart' as http;
// import 'dart:math' as math;
// // import 'package:mobi_tv_entertainment/components/home_screen_pages/tv_show/tv_show_second_page.dart'; // Iski zaroorat nahi
// import 'package:mobi_tv_entertainment/components/home_screen_pages/tv_show_pak/tv_show_second_page.dart'; // Sahi import
// import 'package:mobi_tv_entertainment/main.dart';
// import 'package:mobi_tv_entertainment/components/provider/color_provider.dart';
// import 'package:mobi_tv_entertainment/components/provider/focus_provider.dart';
// import 'package:provider/provider.dart';
...
//       language: json['language'],
//       status: json['status'] ?? 0,
//       order: json['order'] ?? 9999, // Provide default high value for sorting
//       createdAt: json['created_at'],
//       updatedAt: json['updated_at'],
//       deletedAt: json['deleted_at'],
//     );
...
//       return await _fetchFreshTVShows(prefs);
//     } catch (e) {
//       print('❌ Error in getAllTVShows (Pak): $e');
...
//         final cachedTVShows = await _getCachedTVShows(prefs);
//         if (cachedTVShows.isNotEmpty) {
//           print('🔄 Returning cached data as fallback (Pak)');
//           return cachedTVShows;
//         }
//       } catch (cacheError) {
//         print('❌ Cache fallback also failed (Pak): $cacheError');
//       }
...
//       // Sort by order field
//       tvShows.sort((a, b) => a.order.compareTo(b.order));

//       print(
//           '📦 Successfully loaded ${tvShows.length} active Pakistani TV shows from cache');
//       return tvShows;
//     } catch (e) {
//       print('❌ Error loading cached Pakistani TV shows: $e');
//       return [];
...
//         // Cache the fresh data (save all shows, but return only active ones)
//         await _cacheTVShows(prefs, jsonData); // Cache original API response
...
//   Map<String, FocusNode> tvshowsPakFocusNodes = {};
//   FocusNode? _viewAllFocusNode;
//   FocusNode? _firstTVShowFocusNode; // Provider mein register karne ke liye
//   bool _hasReceivedFocusFromReligious = false; // Updated variable name
...
//       );
//     } catch (e) {
//       print('Error scrolling in pak tv shows: $e');
//     }
//   }

//   // // Yeh function ab seedha _firstTVShowFocusNode ko register karta hai
//   // void _setupTVShowsFocusProvider() {
//   //   WidgetsBinding.instance.addPostFrameCallback((_) {
//   //     if (mounted && tvShowsList.isNotEmpty) {
//   //       try {
...
//   //         if (_firstTVShowFocusNode != null) {
//   //           // Node ko provider mein 'tvShowsPak' ID se register karein
//   //           focusProvider.registerFocusNode('tvShowPak', _firstTVShowFocusNode!); // ID Changed
//   //           print(
//   //               '✅ Pakistani TV Shows first focus node registered: ${tvShowsList[0].name}');
...
//   //             if (_firstTVShowFocusNode!.hasFocus &&
//   //                 !_hasReceivedFocusFromReligious) { // Updated variable
//   //               _hasReceivedFocusFromReligious = true;
//   //               setState(() {
//   //                 focusedIndex = 0;
//   //               });
//   //               _scrollToPosition(0);
//   //               print(
//   //                   '✅ Pakistani TV Shows received focus from Religious and scrolled');
//   //             }
//   //           });
//   //         }
//   //       } catch (e) {
...
//           if (!tvshowsPakFocusNodes.containsKey(firstTVShowId)) {
//             tvshowsPakFocusNodes[firstTVShowId] = FocusNode();
//              print('⚠️ First Pak TV show focus node was missing, created it.'); // Added debug log
//           }
//           _firstTVShowFocusNode = tvshowsPakFocusNodes[firstTVShowId];
...
//                if (mounted && _firstTVShowFocusNode!.hasFocus) {

//                  // _hasReceivedFocusFromReligious flag ko sirf tab set karein jab
//                  // focus pehli baar row mein aa रहा है.
//                  if (!_hasReceivedFocusFromReligious) { // Updated variable name
//                    _hasReceivedFocusFromReligious = true;
//                  }
...
//                  _scrollToPosition(0);
//                  print('✅ Pakistani TV Shows (first item) received focus and scrolled');
//                }
//                // ✅ ==========================================================
...
//             });
//           } else {
//              print('❌ ERROR: _firstTVShowFocusNode is null after attempting creation!');
//           }
//         } catch (e) {
//           print('❌ Pakistani TV Shows focus provider setup failed: $e');
//         }
//       } else {
//          print('ℹ️ Pak TV SetupFocusProvider skipped: mounted=$mounted, tvShowsList empty=${tvShowsList.isEmpty}');
//       }
//     });
//   }
...
//           _setupTVShowsFocusProvider(); // *Uske baad* provider ko setup karein
//           _headerAnimationController.forward();
//           _listAnimationController.forward();
//           _debugCacheInfo();
//         }
...
//     for (int i = 0; i < tvShowsList.length && i < maxHorizontalItems; i++) {
//       String tvShowId = tvShowsList[i].id.toString();
//       tvshowsPakFocusNodes[tvShowId] = FocusNode(); // Naya node banayein

//       // Pehle node ke alawa baaki nodes ke liye listener yahin add karein
//       // (Pehle node ka listener _setupTVShowsFocusProvider mein add hoga)
//       if (i > 0) {
//         tvshowsPakFocusNodes[tvShowId]!.addListener(() {
//            if (!mounted) return; // Add mount check
//           if (tvshowsPakFocusNodes[tvShowId]!.hasFocus) {
//             setState(() {
//               focusedIndex = i;
//               _hasReceivedFocusFromReligious = true; // Updated variable
//             });
//             _scrollToPosition(i);
//             print(
//                 '✅ Pakistani TV Show $i focused and scrolled: ${tvShowsList[i].name}');
...
//       TVShowModel tvShow, int index, double screenWidth, double screenHeight) {
//     String tvShowId = tvShow.id.toString();
//     FocusNode? focusNode = tvshowsPakFocusNodes[tvShowId];
...
//               if (index < maxHorizontalItems - 1 && index < tvShowsList.length - 1) {
//                 String nextTVShowId = tvShowsList[index + 1].id.toString();
//                  if (tvshowsPakFocusNodes.containsKey(nextTVShowId)) {
...
//               if (index > 0) {
//                 String prevTVShowId = tvShowsList[index - 1].id.toString();
//                 if (tvshowsPakFocusNodes.containsKey(prevTVShowId)) {
...
//             setState(() {
//               focusedIndex = -1;
//               _hasReceivedFocusFromReligious = false; // Updated variable
//             });
//             context.read<ColorProvider>().resetColor();
...
//           // ❗️ Special Case for Arrow Down: Last row, no navigation
//           else if (key == LogicalKeyboardKey.arrowDown) {
//              // Since this is the last row, we don't navigate further down.
//              // We can optionally provide some feedback or just ignore.
//              print("Already on the last row (Pakistani TV Shows).");
//              // You could potentially loop focus back to the top or do nothing.
//              // For now, let's just handle it to prevent default behavior.

//              // Example: Loop back to the first row (Banner/WatchNow)
//              // setState(() {
//              //   focusedIndex = -1;
//              //   _hasReceivedFocusFromReligious = false;
//              // });
//              // context.read<ColorProvider>().resetColor();
//              // FocusScope.of(context).unfocus();
//              // context.read<FocusProvider>().requestFocus('watchNow'); // Go to first row

//              // Or simply do nothing:
//              return KeyEventResult.handled; // Still handle to prevent system sounds etc.

//             // Old logic using focusNextRow (will fail gracefully if no next row)
//             // setState(() {
//             //   focusedIndex = -1;
//             //   _hasReceivedFocusFromReligious = false; // Updated variable
//             // });
//             // context.read<ColorProvider>().resetColor();
//             // FocusScope.of(context).unfocus(); // Unfocus current item
//             // context.read<FocusProvider>().focusNextRow(); // Ask provider (will do nothing if this is last)
//             // return KeyEventResult.handled;
...
//   Widget _buildTVShowsList(double screenWidth, double screenHeight) {
//     bool showViewAll = tvShowsList.length > maxHorizontalItems;
//     int itemCount = math.min(tvShowsList.length, maxHorizontalItems);
...
//              if (tvShowsList.isNotEmpty && tvShowsList.length >= maxHorizontalItems) {
//                String tvShowId = tvShowsList[maxHorizontalItems - 1].id.toString();
//                if (tvshowsPakFocusNodes.containsKey(tvShowId)) {
...
//           else if (key == LogicalKeyboardKey.arrowDown) {
//               // Since this is the last row, we don't navigate further down.
//              print("Already on the last row (Pakistani TV Shows - View All).");
//              // Loop back to top?
//              // setState(() {
//              //   focusedIndex = -1;
//              //   _hasReceivedFocusFromReligious = false;
...
//           onTap: _navigateToGridPage,
//           totalItems: tvShowsList.length,
//           itemType: 'PAKISTANI TV SHOWS', // Specific type
//         ),
//       ),
//     );
//   }
...
// // ==========================================================
// // (ProfessionalPakistaniTVShowCard, ProfessionalPakistaniTVShowViewAllButton, ProfessionalPakistaniTVShowLoadingIndicator, etc.)
...
//             if (gridFocusedIndex >= 0 && gridFocusedIndex < widget.tvShowsList.length) {
//               _navigateToTVShowDetails(
//                 widget.tvShowsList[gridFocusedIndex],
//                 gridFocusedIndex,
//               );
...
//           children: [
//             const Icon(
//               Icons.live_tv_outlined, // Use the specific icon
...
//     if (widget.tvShow.logo != null && widget.tvShow.logo!.isNotEmpty) {
...
//         },
//         errorBuilder: (context, error, stackTrace) {
//            print('❌ Error loading grid pak tv image: ${widget.tvShow.logo}');
//            return placeholder;
//         },
//       );
//     } else {
...
//                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
//                      decoration: BoxDecoration(
//                        color: ProfessionalColors.accentGreen.withOpacity(0.3), // Language color
//                        borderRadius: BorderRadius.circular(8),
//                        border: Border.all(
//                          color: ProfessionalColors.accentGreen.withOpacity(0.5),
//                          width: 1,
...
//                      ),
//                      child: Text(
//                        widget.tvShow.displayLanguage, // Use displayLanguage getter
//                        style: const TextStyle(
//                          color: ProfessionalColors.accentGreen,
//                          fontSize: 8,
//                          fontWeight: FontWeight.bold,
...
//                      ),
//                    ),
//                     // Add more badges here if needed (like channel type)
//                     SizedBox(width: 8),
//                     Container(
//                       padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
//                       decoration: BoxDecoration(
...
//                       ),
//                       child: Text(
//                         widget.tvShow.channelType, // Use channelType getter
//                         style: TextStyle(
//                           color: _dominantColor,
//                           fontSize: 8,
//                           fontWeight: FontWeight.bold,
...
import 'package:mobi_tv_entertainment/components/home_screen_pages/tv_show/tv_show_slider_screen.dart'; // ✅ Reuse existing page
...
// ✅ ==========================================================
// ✅ [NEW] TVShowNetworkPakService with Caching
// ✅ ==========================================================
class TVShowNetworkPakService {
  ...
  static Future<List<TVShowNetworkPakModel>> getAllTVShowPakNetworks(
      {bool forceRefresh = false}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      // Check if we should use cache
      if (!forceRefresh && await _shouldUseCache(prefs)) {
        print('📦 Loading TV Shows Pak Networks from cache...'); // ✅ MODIFIED
        final cachedNetworks = await _getCachedTVShowPakNetworks(prefs);
        if (cachedNetworks.isNotEmpty) {
          print(
              '✅ Successfully loaded ${cachedNetworks.length} TV shows pak networks from cache'); // ✅ MODIFIED
      ...
      // Load fresh data if no cache or force refresh
      print('🌐 Loading fresh TV Shows Pak Networks from API...'); // ✅ MODIFIED
      return await _fetchFreshTVShowPakNetworks(prefs);
    } catch (e) {
      print('❌ Error in getAllTVShowPakNetworks: $e'); // ✅ MODIFIED
      try {
        final prefs = await SharedPreferences.getInstance();
        final cachedNetworks = await _getCachedTVShowPakNetworks(prefs);
        if (cachedNetworks.isNotEmpty) {
          print(
              '🔄 Returning cached TV shows pak networks as fallback'); // ✅ MODIFIED
          return cachedNetworks;
        }
      } catch (cacheError) {
        print('❌ Cache fallback also failed: $cacheError');
      }
      throw Exception('Failed to load TV shows pak networks: $e'); // ✅ MODIFIED
    }
  }
  ...
  static Future<bool> _shouldUseCache(SharedPreferences prefs) async {
    try {
      ...
      print(
          '❌ Error checking TV Shows Pak Networks cache validity: $e'); // ✅ MODIFIED
      return false;
    }
  }

  /// Get Networks from cache
  static Future<List<TVShowNetworkPakModel>> _getCachedTVShowPakNetworks(
      SharedPreferences prefs) async {
    try {
      // ✅ [MODIFIED] Use new key variable
      final cachedData = prefs.getString(_cacheKeyTVShowsPak);
      if (cachedData == null || cachedData.isEmpty) {
        print('📦 No cached TV Shows Pak Networks data found'); // ✅ MODIFIED
        return [];
      }
      ...
      final networks = jsonData
          .map((json) =>
              TVShowNetworkPakModel.fromJson(json as Map<String, dynamic>))
          .where((network) => network.status == 1) // Filter active networks
          .toList();

      print(
          '📦 Successfully loaded ${networks.length} TV shows pak networks from cache'); // ✅ MODIFIED
      return networks;
    } catch (e) {
      print('❌ Error loading cached TV shows pak networks: $e'); // ✅ MODIFIED
      return [];
    }
  }

  /// Fetch fresh Networks from API and cache them
  static Future<List<TVShowNetworkPakModel>> _fetchFreshTVShowPakNetworks(
      SharedPreferences prefs) async {
    try {
      ...
      final response = await https
          .post(url,
            ...
            // ✅ [MODIFIED] data_for changed to 'tvshowspak'
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '✅ Successfully loaded ${activeNetworks.length} active TV shows pak networks from API (from ${allNetworks.length} total)'); // ✅ MODIFIED
      ...
      print('❌ Error fetching fresh TV shows pak networks: $e'); // ✅ MODIFIED
      rethrow;
    }
  }

  /// Cache Networks data
  static Future<void> _cacheTVShowPakNetworks(
    ...
    try {
      ...
      await Future.wait([
        // ✅ [MODIFIED] Use new key variable
        prefs.setString(_cacheKeyTVShowsPak, jsonString),
        prefs.setString(_cacheKeyTimestamp, currentTimestamp),
      ]);

      print(
          '💾 Successfully cached ${networkData.length} TV shows pak networks'); // ✅ MODIFIED
    } catch (e) {
      print('❌ Error caching TV shows pak networks: $e'); // ✅ MODIFIED
    }
  }
  ...
  static void _loadFreshDataInBackground() {
    Future.delayed(const Duration(milliseconds: 500), () async {
      try {
        print(
            '🔄 Loading fresh TV shows pak networks data in background...'); // ✅ MODIFIED
        final prefs = await SharedPreferences.getInstance();
        await _fetchFreshTVShowPakNetworks(prefs);
        print(
            '✅ TV shows pak networks background refresh completed'); // ✅ MODIFIED
      } catch (e) {
        print(
            '⚠️ TV shows pak networks background refresh failed: $e'); // ✅ MODIFIED
      }
    });
  }
  ...
  static Future<void> clearCache() async {
    try {
      ...
      await Future.wait([
        // ✅ [MODIFIED] Use new key variable
        prefs.remove(_cacheKeyTVShowsPak),
        prefs.remove(_cacheKeyTimestamp),
      ]);
      print(
          '🗑️ TV shows pak networks cache cleared successfully'); // ✅ MODIFIED
    } catch (e) {
      print('❌ Error clearing TV shows pak networks cache: $e'); // ✅ MODIFIED
    }
  }
    ...
    try {
      ...
      if (timestampStr == null || cachedData == null) {
        return {
          'hasCachedData': false,
          'cacheAge': 0,
          'cachedTVShowsPakNetworksCount': 0, // ✅ MODIFIED
          'cacheSize': 0
        };
      }
      ...
      return {
        'hasCachedData': true,
        'cacheAge': cacheAgeMinutes,
        'cachedTVShowsPakNetworksCount': jsonData.length, // ✅ MODIFIED
        'cacheSize': cacheSizeKB,
        'isValid': cacheAge < _cacheDurationMs,
      };
    } catch (e) {
      print(
          '❌ Error getting TV shows pak networks cache info: $e'); // ✅ MODIFIED
      return {'hasCachedData': false, 'error': e.toString()};
    }
  }

  /// Force refresh data (bypass cache)
  static Future<List<TVShowNetworkPakModel>> forceRefresh() async {
    print('🔄 Force refreshing TV shows pak networks data...'); // ✅ MODIFIED
    return await getAllTVShowPakNetworks(forceRefresh: true);
  }
}

// ✅ ==========================================================
// ✅ [NEW] Main Widget: ManageTvShowsPak
// ✅ ==========================================================
...
class _TvShowsPakState extends State<TvShowsPak>
  ...
  void initState() {
    ...
    fetchTVShowPakNetworksWithCache(); // ✅ [NEW]
  }
  ...
  void dispose() {
    ...
    if (_fullTVShowPakNetworkList.isNotEmpty) {
      firstNetworkId = _fullTVShowPakNetworkList[0].id.toString();
    }
  ...
  void _initializeFocusListeners() {
    ...
    print('✅ TV Shows Pak Network focus listeners initialized');
  }
  ...
  void _onViewAllFocusChange() {
    if (mounted && _viewAllFocusNode.hasFocus) {
      setState(() {
        focusedIndex =
            _displayedTVShowPakNetworkList.length; // View All ka index
      });
  ...
  void _setupTVShowPakNetworkFocusProvider() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _displayedTVShowPakNetworkList.isNotEmpty) {
        try {
          final focusProvider =
              Provider.of<FocusProvider>(context, listen: false);
          final firstNetworkId =
              _displayedTVShowPakNetworkList[0].id.toString();
          _firstTVShowPakNetworkFocusNode =
              tvShowPakNetworkFocusNodes[firstNetworkId];

          if (_firstTVShowPakNetworkFocusNode != null) {
            // ✅ [MODIFIED] Register with new unique key 'tvShowsPak'
            focusProvider.registerFocusNode(
                'tvShowPak', _firstTVShowPakNetworkFocusNode!);
            print(
                '✅ TV Shows Pak Networks first focus node registered: ${_displayedTVShowPakNetworkList[0].name}');
            ...
            _firstTVShowPakNetworkFocusNode!.addListener(() {
              if (mounted && _firstTVShowPakNetworkFocusNode!.hasFocus) {
                ...
                setState(() => focusedIndex = 0);
                _scrollToPosition(0);
                print('✅ TV Shows Pak Network (first item) received focus');
              }
            });
          }
        } catch (e) {
          print('❌ TV Shows Pak Networks focus provider setup failed: $e');
        }
      }
    });
  }
  ...
  Future<void> fetchTVShowPakNetworksWithCache() async {
    ...
    try {
      final fetchedNetworks =
          await TVShowNetworkPakService.getAllTVShowPakNetworks();

      if (mounted) {
        _fullTVShowPakNetworkList = fetchedNetworks;
        // View All Logic
        if (_fullTVShowPakNetworkList.length > 10) {
          _displayedTVShowPakNetworkList =
              _fullTVShowPakNetworkList.sublist(0, 10);
        } else {
        ...
        if (_fullTVShowPakNetworkList.isNotEmpty) {
          _createFocusNodesForItems();
          _setupTVShowPakNetworkFocusProvider(); // ✅ [NEW]
          _headerAnimationController.forward();
          _listAnimationController.forward();
          _debugCacheInfo();
        }
  ...
  Future<void> _debugCacheInfo() async {
    try {
      final cacheInfo = await TVShowNetworkPakService.getCacheInfo(); // ✅ [NEW]
      print('📊 TV Shows Pak Networks Cache Info: $cacheInfo'); // ✅ [NEW]
    } catch (e) {
      print('❌ Error getting TV Shows Pak Networks cache info: $e'); // ✅ [NEW]
    }
  }
  ...
  void _createFocusNodesForItems() {
    ...
    for (int i = 0; i < _displayedTVShowPakNetworkList.length; i++) {
      ...
      if (i > 0) {
        // Pehle node ka listener _setupTVShowPakNetworkFocusProvider mein hai
    ...
    print('✅ Created ${tvShowPakNetworkFocusNodes.length} network focus nodes');
  }

  // ✅ [NEW] Navigation method (commented out, but renamed for consistency)
  void _navigateToTVShowPakNetworkDetails(TVShowNetworkPakModel network) async {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        ...
        builder: (context) => TvShowPakSliderScreen(
    ...
    //   Future.delayed(Duration(milliseconds: 300), () {
    //     if (mounted) {
    //       int currentIndex = _displayedTVShowPakNetworkList
    //           .indexWhere((net) => net.id == network.id);
    //       if (currentIndex != -1) {
    //         String networkId = network.id.toString();
    //         if (tvShowPakNetworkFocusNodes.containsKey(networkId)) {
    //           setState(() {
    //             focusedIndex = currentIndex;
    //             _hasReceivedFocus = true;
    //           });
    //           tvShowPakNetworkFocusNodes[networkId]!.requestFocus();
    //           _scrollToPosition(currentIndex);
  ...
  // ✅ [NEW] Navigate to Grid Page (View All)
  // Reuses TvShowSliderScreen
  void _navigateToGridPage() {
    print(
        'Navigating to Grid Page for all ${_fullTVShowPakNetworkList.length} Networks...');
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TvShowPakSliderScreen(
  ...
  // Reuses TvShowSliderScreen
  void _navigateToGridPageWithNetwork(TVShowNetworkPakModel network) {
    print(
        'Navigating to Grid Page, pre-selecting: ${network.name} (ID: ${network.id})');

    int returnIndex =
        _displayedTVShowPakNetworkList.indexWhere((n) => n.id == network.id);

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TvShowPakSliderScreen(
          initialNetworkId: network.id, // ✅ ID PASS KAREIN
        ),
      ),
    );
    // .then((_) {
    ...
    //       if (returnIndex != -1) {
    //         String networkId =
    //             _displayedTVShowPakNetworkList[returnIndex].id.toString();
    //         if (tvShowPakNetworkFocusNodes.containsKey(networkId)) {
    //           tvShowPakNetworkFocusNodes[networkId]!.requestFocus();
    //         }
  ...
  // ✅ [NEW] Build item
  Widget _buildTVShowNetworkItem(TVShowNetworkPakModel network, int index,
    ...
    return Focus(
      ...
      onFocusChange: (hasFocus) async {
        ...
        if (hasFocus) {
          ...
          bool isAnyItemFocused =
              tvShowPakNetworkFocusNodes.values.any((node) => node.hasFocus);
          if (!mounted) return;
          ...
          if (key == LogicalKeyboardKey.arrowRight) {
            if (index < _displayedTVShowPakNetworkList.length - 1) {
              String nextNetworkId =
                  _displayedTVShowPakNetworkList[index + 1].id.toString();
              FocusScope.of(context)
                  .requestFocus(tvShowPakNetworkFocusNodes[nextNetworkId]);
            ...
            if (index > 0) {
              String prevNetworkId =
                  _displayedTVShowPakNetworkList[index - 1].id.toString();
              FocusScope.of(context)
                  .requestFocus(tvShowPakNetworkFocusNodes[prevNetworkId]);
            }
            return KeyEventResult.handled;
          } else if (key == LogicalKeyboardKey.arrowUp) {
            setState(() {
            ...
            Future.delayed(const Duration(milliseconds: 50), () {
              if (mounted) {
                try {
                  // // ✅ [MODIFIED] Aapko yeh key apne upar wali row ke hisab se set karni hogi
                  // Provider.of<FocusProvider>(context, listen: false)
                  //     .requestFocus('row_above_tvshowspak'); // 👈 ISSE CHECK KAREIN
                  // print('✅ Navigating UP from TV Shows Pak row');
                  context.read<FocusProvider>().focusPreviousRow();
                } catch (e) {
            ...
            Future.delayed(const Duration(milliseconds: 50), () {
              if (mounted) {
                try {
                  // // ✅ [MODIFIED] Aapko yeh key apne neeche wali row ke hisab se set karni hogi
                  // Provider.of<FocusProvider>(context, listen: false)
                  //     .requestFocus('row_below_tvshowspak'); // 👈 ISSE CHECK KAREIN
                  // print('✅ Navigating DOWN from TV Shows Pak row');
            ...
            return KeyEventResult.handled;
              ...
              key == LogicalKeyboardKey.select) {
            _navigateToGridPageWithNetwork(
                network); // ✅ Navigates to slider screen
            return KeyEventResult.handled;
          }
        }
        return KeyEventResult.ignored;
      ...
      child: GestureDetector(
        onTap: () => _navigateToGridPageWithNetwork(
            network), // ✅ Navigates to slider screen
  ...
  Widget _buildViewAllButton(double screenWidth, double screenHeight) {
    return Focus(
      ...
      onFocusChange: (hasFocus) {
        ...
        if (hasFocus) {
          setState(() {
            focusedIndex = _displayedTVShowPakNetworkList.length;
            _hasReceivedFocus = true;
          });
          context
              .read<ColorProvider>()
        ...
        if (event is RawKeyDownEvent) {
          ...
          if (key == LogicalKeyboardKey.arrowLeft) {
            if (_displayedTVShowPakNetworkList.isNotEmpty) {
              String prevNetworkId =
                  _displayedTVShowPakNetworkList.last.id.toString();
              FocusScope.of(context)
                  .requestFocus(tvShowPakNetworkFocusNodes[prevNetworkId]);
            }
            return KeyEventResult.handled;
            ...
            Future.delayed(const Duration(milliseconds: 50), () {
              if (mounted) {
                try {
                  ...
                  // Provider.of<FocusProvider>(context, listen: false)
                  //     .requestFocus('row_above_tvshowspak'); // 👈 ISSE CHECK KAREIN
                  // print('✅ Navigating UP from TV Shows Pak row (View All)');
                  context.read<FocusProvider>().focusPreviousRow();
                } catch (e) {
                  print('❌ Failed to navigate UP: $e');
                }
            ...
            Future.delayed(const Duration(milliseconds: 50), () {
              if (mounted) {
                try {
                  ...
                  // Provider.of<FocusProvider>(context, listen: false)
                  //     .requestFocus('row_below_tvshowspak'); // 👈 ISSE CHECK KAREIN
                  // print('✅ Navigating DOWN from TV Shows Pak row (View All)');
                  context.read<FocusProvider>().focusPreviousRow();
                } catch (e) {
                  print('❌ Failed to navigate DOWN: $e');
                }
    ...
    return FadeTransition(
      ...
      child: Container(
        ...
        child: ListView.builder(
          ...
          cacheExtent: 9999,
          itemCount:
              _displayedTVShowPakNetworkList.length + (_showViewAll ? 1 : 0),
          itemBuilder: (context, index) {
            if (index < _displayedTVShowPakNetworkList.length) {
              ...
              return _buildTVShowNetworkItem(
                  network, index, screenWidth, screenHeight); // ✅ [NEW]
            } else {
        ...
        child: Row(
          ...
          children: [
            ShaderMask(
              ...
              child: const Text(
                'PAK TV SHOWS', // ✅ [MODIFIED] Title Updated
                style: TextStyle(
                  fontSize: 24,
                  color: Colors.white,
                  fontWeight: FontWeight.w700,
  ...
  Widget _buildBody(double screenWidth, double screenHeight) {
    if (isLoading) {
      return ProfessionalTVShowNetworkPakLoadingIndicator(
          // ✅ [NEW]
          message: 'Loading Pak TV Shows...'); // ✅ [NEW]
    } else if (_fullTVShowPakNetworkList.isEmpty) {
      return _buildEmptyWidget();
    } else {
      return _buildTVShowNetworksList(screenWidth, screenHeight); // ✅ [NEW]
    }
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          const Text(
            'No Pak TV Shows Found', // ✅ [MODIFIED] Text Updated
            style: TextStyle(
              color: ProfessionalColors.textPrimary,
              fontSize: 18,
              fontWeight: FontWeight.w600,
...
// ✅ [NEW] Placeholder Grid Page (Agar aap TvShowSliderScreen ka istemaal nahi karna chahte)
// Yeh page TvShowSliderScreen se replace ho gaya hai, lekin code yahan hai agar zaroorat pade.
class TVShowNetworkPakGridPage extends StatelessWidget {
  final List<TVShowNetworkPakModel> networks;
  const TVShowNetworkPakGridPage({Key? key, required this.networks})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            Text('All Pak TV Show Networks (${networks.length})'), // ✅ MODIFIED

...
//   @override
//   _TvShowPakFinalDetailsPageState createState() =>
//       _TvShowPakFinalDetailsPageState();
// }
...
//       // **MODIFIED: Automatically load episodes for the first season**
...
//     return Positioned(
//       top: 80, // **MODIFIED: Adjusted top spacing**
//       left: 0,
//       right: 0,
...
//     return Container(
//       margin: const EdgeInsets.fromLTRB(20, 20, 20, 20), // Added bottom margin
//       child: Row(
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           // Left Panel - Seasons
...
class _TvShowPakFinalDetailsPageState extends State<TvShowPakFinalDetailsPage>
  ...
  Future<void> _fetchSeasonsFromAPI() async {
    ...
    try {
      String authKey = SessionManager.authKey;
      var url =
          Uri.parse(SessionManager.baseUrl + 'getShowSeasonsPak/${widget.id}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody.startsWith('[') || responseBody.startsWith('{')) {
          final List<dynamic> data = jsonDecode(responseBody);
          final seasons =
              data.map((season) => ShowSeasonModel.fromJson(season)).toList();
  ...
  Future<void> _fetchEpisodes(int seasonId) async {
    // If we already have the episodes in memory, just switch view
    // You can remove this check if you want to force reload every time you click a season
    if (_filteredEpisodesMap.containsKey(seasonId)) {
      setState(() {
        _selectedSeasonIndex =
            _filteredSeasons.indexWhere((season) => season.id == seasonId);
    ...
    try {
      String authKey = SessionManager.authKey;
      var url = Uri.parse(
          SessionManager.baseUrl + 'getShowSeasonsEpisodesPak/$seasonId');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody.startsWith('[') || responseBody.startsWith('{')) {
          final List<dynamic> data = jsonDecode(responseBody);
          final episodes =
              data.map((e) => ShowEpisodeModel.fromJson(e)).toList();
          ...
          throw Exception("Invalid episode response format");
        }
      } else {
        throw Exception('Failed to load episodes for season $seasonId');
      }
  ...
  Future<void> _refreshDataOnReturn() async {
    print('🔄 Refreshing data on return from video player');
    // Since we removed caching, we simply reload the current data to ensure progress is updated
    if (_filteredSeasons.isNotEmpty &&
        _selectedSeasonIndex < _filteredSeasons.length) {
      final currentSeasonId = _filteredSeasons[_selectedSeasonIndex].id;
  ...
  // Enhanced image widget builder without custom caching logic
  Widget _buildEnhancedImage({
    required String imageUrl,
    required double width,
    required double height,

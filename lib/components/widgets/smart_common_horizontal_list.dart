...
import 'package:mobi_tv_entertainment/components/provider/color_provider.dart';
import 'package:mobi_tv_entertainment/components/provider/focus_provider.dart';
import 'package:mobi_tv_entertainment/plan_expired_screen.dart'; // ✅ Make sure this import matches your path
...
class SmartCommonHorizontalList extends StatefulWidget {
  ...
  // ✅ NEW CALLBACK: Parent (Dashboard) ko days update karne ke liye
  final void Function(int daysLeft)? onPlanExpiringUpdate; 
...
class _SmartCommonHorizontalListState extends State<SmartCommonHorizontalList> with AutomaticKeepAliveClientMixin, TickerProviderStateMixin {
  ...
  // // ✅ 1. SECURE VERIFICATION FUNCTION
  // Future<bool> _verifyPlanStatusLocally() async {
  //   final String? authKey = SessionManager.authKey;
  //   if (authKey == null || authKey.isEmpty) return true; // Let standard auth logic handle this
  ...
  //       if (planExpired) {
  //         if (mounted) {
  //           // Plan expired -> Fling user to Expiry Screen and clear history
  ...
  //       // Plan is expiring soon -> Alert Dashboard
  //       if (daysLeft <= 3 && widget.onPlanExpiringUpdate != null) {
  //         widget.onPlanExpiringUpdate!(daysLeft);
  //       }
  //     }
  //   } catch (e) {
  //     print("Error verifying plan status on click: $e");
  //   }
  //   return true; // If network fails, allow play to prevent bad UX for active users
  // }
  ...
  Future<bool> _verifyPlanStatusLocally() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // ✅ DISPATCH NOTIFICATION TO UPDATE DASHBOARD BANNER SLIDER IMMIDIATELY
        if (mounted) {
        ...
        // Keep old callback trigger just in case
        if (daysLeft <= 3 && widget.onPlanExpiringUpdate != null) {
          widget.onPlanExpiringUpdate!(daysLeft);
        }
      }
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          // ✅ ADDED A SMALL LOADING SPINNER IF WE ARE VERIFYING AND NAVIGATING
          if (_isNavigating)
          ...
          if (_isSectionFocused && !_isNavigating) _buildShadowOverlay(),
        ],
      ),
    );
  }

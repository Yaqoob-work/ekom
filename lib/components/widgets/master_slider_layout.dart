...
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:http/http.dart' as https; // ✅ ADDED HTTP FOR API CALL
import 'package:mobi_tv_entertainment/components/provider/focus_provider.dart';
import 'package:mobi_tv_entertainment/components/widgets/focused_overlay_list.dart';
import 'package:mobi_tv_entertainment/main.dart';
...
// ✅ MAKE SURE THIS IMPORT MATCHES YOUR APP'S PATH
...
class MasterSliderLayout<T> extends StatefulWidget {
  ...
  const MasterSliderLayout({
    ...
    required this.cardHeight,
    this.shouldShuffle = false,
    this.onPlanExpiringUpdate, // ✅ INITIALIZED
  }) : super(key: key);
...
class _MasterSliderLayoutState<T> extends State<MasterSliderLayout<T>>
  ...
  bool _isCheckingPlan = false; // ✅ LOADER STATE FOR API CHECK
  ...
  // ✅ 1. NEW SECURE VERIFICATION FUNCTION
  Future<bool> _verifyPlanStatusLocally() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (planExpired) {
          if (mounted) {
            // Plan expired -> Fling user to Expiry Screen
        ...
        // Plan is expiring soon -> Alert Parent
  ...
  Future<void> _handleSafeNavigation(T item, int index) async {
    if (_isCheckingPlan) return; 
    ...
    if (!isPlanSafe) return; // Block navigation if plan is expired

    // Play the content
    widget.onContentTap(item, index);
  }
  ...
  KeyEventResult _navigateItems(LogicalKeyboardKey key) {
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      ...
      // ✅ CHANGED: USED SAFE NAVIGATION HERE INSTEAD OF DIRECT CALL
      _handleSafeNavigation(_displayList[i], i);
      ...
      body: Focus(
        ...
        child: Stack(
          children: [
            ...
            // ✅ NEW LOADING OVERLAY FOR API CHECK
            if (_isCheckingPlan)
              Positioned.fill(
                child: Container(
      ...
      child: FocusedOverlayList(
        ...
        itemBuilder: (ctx, i) {
          ...
          return _MasterSliderCard<T>(
            ...
            onTap: () => _handleSafeNavigation(item, i),
            onFocusChange: (has) {
              if (has && !_isDisposed) {
                if (_focusedItemIndex != i) {
                  setState(() => _focusedItemIndex = i);

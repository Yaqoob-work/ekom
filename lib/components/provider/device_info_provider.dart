...
import 'package:flutter/foundation.dart' show kIsWeb; // ✅ Ye zaroori hai
import 'package:device_info_plus/device_info_plus.dart';
...
class DeviceInfoProvider with ChangeNotifier {
  ...
  Future<void> loadDeviceInfo() async {
    ...
    try {
      // ✅ SABSE PEHLE WEB CHECK KAREIN
      if (kIsWeb) {
        final WebBrowserInfo webInfo = await deviceInfoPlugin.webBrowserInfo;
        deviceIdentifier = 'Web Browser: ${webInfo.browserName.toString().split('.').last}';
        print("--- 2. RAW DEVICE DATA: WEB BROWSER ---");
      } 
      // Agar Web nahi hai, tabhi Android check karein
      ...
      // Agar Web aur Android dono nahi, tab iOS check karein
      else if (Platform.isIOS) {
        final IosDeviceInfo iosInfo = await deviceInfoPlugin.iosInfo;
        deviceIdentifier = iosInfo.name;
      }

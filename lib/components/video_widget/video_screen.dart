...
void didChangeAppLifecycleState(AppLifecycleState state) {
  ...
  Future<void> _initializeVLCController(String baseUrl) async {
    ...
    try {
      ...
      _controller = VlcPlayerController.network(
        fullVlcUrl,
        // FIX 1: Change Hardware Acceleration. 
        // 'HwAcc.disabled' forces software decoding, bypassing TV hardware scaling bugs.
        // If the video lags, try 'HwAcc.full' instead, but never use 'auto' on TV.
        ...
        options: VlcPlayerOptions(
          ...
          // FIX 2: Pass raw arguments to the VLC C++ engine
          extras: [
            '--no-autoscale',       // Strictly forbids VLC from zooming the texture
            '--video-scale=1.0',    // Locks the scale multiplier
            '--aspect-ratio=16:9',  // Forces a standard TV shape
          ],
        ),
      );
  ...
  // Widget _buildVideoPlayer() {
  //   if (!_isVideoInitialized || _controller == null) return const Center(child: CircularProgressIndicator());
    
  //   return LayoutBuilder(
  //     builder: (context, constraints) {
  //       double vW = _controller!.value.size.width;
  //       double vH = _controller!.value.size.height;
  //       if (vW <= 0 || vH <= 0) { vW = 16; vH = 9; }

  //       final vRatio = vW / vH;
  //       final sRatio = constraints.maxWidth / constraints.maxHeight;
  //       double sX = 1.0, sY = 1.0;

  //       if (vRatio < sRatio) { sX = sRatio / vRatio; } 
  //       else { sY = vRatio / sRatio; }

  //       // Limit stretch to prevent weird distortion
  //       sX = sX.clamp(1.0, 1.3);
  //       sY = sY.clamp(1.0, 1.3);
  ...
  //             scaleX: sX, scaleY: sY,
  //             child: VlcPlayer(
  //               key: ValueKey(_currentModifiedUrl),
  //               controller: _controller!,
  //               aspectRatio: vRatio,
  //               placeholder: const Center(child: CircularProgressIndicator()),
  //             ),
...
//         // Default to standard TV aspect ratio if VLC hasn't reported dimensions yet
//         if (videoWidth <= 0 || videoHeight <= 0) {
//           videoWidth = 16.0;
//           videoHeight = 9.0;
//         }

//         // Calculate the screen's ratio to force the video to fill the bounds exactly
//         final screenRatio = screenWidth / screenHeight;
...
//               controller: _controller!,
//               placeholder: const Center(child: CircularProgressIndicator()),
//               // Passing the screenRatio prevents the video from zooming out of bounds.
//               // It will fill the screen without cutting off channel logos.

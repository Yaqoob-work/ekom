...
//   static Future<Map<String, dynamic>> getMoviesWithPagination({
//     int page = 1,
...
//       print('📡 Fetching movies: page=$page, records=$recordsPerPage');

//       final response = await http.get(
//         Uri.parse('https://acomtv.coretechinfo.com/public/api/getAllMovies?page=$page&records=$recordsPerPage'),
//         headers: {'auth-key': authKey},
//       ).timeout(
//         const Duration(seconds: 30),
//         onTimeout: () {
...
//         final hasMore = movies.length == recordsPerPage;
//         final totalLoaded = (page - 1) * recordsPerPage + movies.length;

//         print('✅ Loaded ${movies.length} movies for page $page (total: $totalLoaded)');
...
//   static Future<List<Movie>> getMoviesForList({bool forceRefresh = false}) async {
//     try {
//       final result = await getMoviesWithPagination(page: 1, recordsPerPage: 8, forceRefresh: forceRefresh);
//       return result['movies'] as List<Movie>;
//     } catch (e) {
//       print('❌ Error in getMoviesForList: $e');
//       rethrow;
//     }
...
//   try {
//     // 🛡️ SAFE LOADING DIALOG with timeout
//     if (mounted) {
...
//       final playlist = [videoData]; // Simple single-item playlist

//       // 🛡️ SAFE PAGE NAVIGATION
//       await Navigator.push(
//         context,
//         PageRouteBuilder(
...
//             // Simple fade transition to avoid memory issues
//             return FadeTransition(
//               opacity: animation,
//               child: child,
//             );
...
//       print('✅ [NAV] Returned from movie player successfully');
//     }
...
//         loadingOverlay.remove();
//       } catch (overlayError) {
//         print('⚠️ [WARNING] Overlay removal failed: $overlayError');
//       }
//     }
...
//       ScaffoldMessenger.of(context).showSnackBar(
//         SnackBar(
//           content: Text('Failed to play movie: ${movie.name}'),
...
//         context,
//         MaterialPageRoute(
//           builder: (context) => CumulativeProgressiveMoviesGrid (
...
//   @override
//   _CumulativeProgressiveMoviesGridState createState() =>
//       _CumulativeProgressiveMoviesGridState();
// }
...
//           _allMovies = movies;
//           _hasMore = hasMore;
//           _loadedRanges = {1}; // Mark range 1 as loaded
//           _currentMaxRange = 1;
...
//             if (mounted && !_isDisposed) {
//               _loadSpecificRange(2); // Load range 2 (movies 51-100)
//             }
//           });
//         }
//       }
//     } catch (e) {
//       print('❌ Error loading initial range: $e');
//       if (mounted && !_isDisposed) {
//         setState(() {
...
//       print('🔮 Preloading range $preloadRange for extra smooth experience...');
//       _loadSpecificRange(preloadRange);
//     }
//   }

//   // 🆕 Load specific range (e.g., range 2 = movies 51-100)
//   Future<void> _loadSpecificRange(int rangeNumber) async {
...
//             Future.delayed(const Duration(milliseconds: 300), () {
//               if (mounted && !_isDisposed) {
//                 _loadSpecificRange(3); // Load range 3 (movies 101-150)
//               }
//             });
//           }
//         } else {
//           print('⚠️ Range $rangeNumber returned no movies');
//           setState(() {
//             _hasMore = false;
//           });
//         }
//       }
//     } catch (e) {
//       print('❌ Error loading range $rangeNumber: $e');
//     } finally {
...
//   // 🎯 Enhanced navigation with cumulative loading
//   void _navigateGrid(LogicalKeyboardKey key) {
//     if (_isInitialLoading || _allMovies.isEmpty || _isDisposed) return;

//     try {
//       int newIndex = _focusedIndex;
//       final int totalItems = _allMovies.length;
//       final int currentRow = _focusedIndex ~/ _columnsCount;
//       final int currentCol = _focusedIndex % _columnsCount;
...
//             newIndex = nextRowIndex;
//           } else {
//             // At edge - try to load more if possible
//             if (_hasMore && !_isLoadingMore) {
//               print('🎯 Edge reached - trying to load more...');
...
//             final int targetIndex = lastRowStartIndex + currentCol;
//             newIndex = targetIndex < totalItems ? targetIndex : totalItems - 1;
//           }
//           break;

//         case LogicalKeyboardKey.arrowUp:
//           if (currentRow > 0) {
//             final int prevRowIndex = (currentRow - 1) * _columnsCount + currentCol;
//             newIndex = prevRowIndex;
//           }
//           break;
//       }

//       // Apply navigation if valid
//       if (newIndex != _focusedIndex && newIndex >= 0 && newIndex < _allMovies.length) {
//         final newMovieId = _allMovies[newIndex].id.toString();
//         if (_movieFocusNodes.containsKey(newMovieId)) {
//           setState(() {
...
//       }
//     } catch (e) {
//       print('❌ Error in grid navigation: $e');
//     }
//   }

//   // 🆕 Get current range info for debugging
//   String _getCurrentRangeInfo(int index) {
//     final range = (index ~/ _batchSize) + 1;
//     final positionInRange = (index % _batchSize) + 1;
//     final movieNumber = index + 1;
//     return '(Movie $movieNumber, Range $range, Position $positionInRange/$_batchSize)';
//   }
...
//         _startStaggeredAnimation();
//         _focusFirstGridItem();
//         print('✅ Grid initialization completed successfully');
//       }
//     } catch (e) {
//       print('❌ Error in post-data initialization: $e');
//     }
//   }
...
//         _movieFocusNodes[movieId] = FocusNode()
//           ..addListener(() {
//             if (mounted && !_isDisposed && _movieFocusNodes.containsKey(movieId)) {
//               if (_movieFocusNodes[movieId]!.hasFocus) {
//                 setState(() {
//                   _focusedIndex = i;
//                 });
//                 _scrollToFocusedItem(movieId);
...
//       print('✅ Created ${_movieFocusNodes.length} focus nodes');
//     } catch (e) {
//       print('❌ Error initializing focus nodes: $e');
//     }
//   }
...
//   // Safe dispose all focus nodes
//   void _safeDisposeAllFocusNodes() {
//     final nodeIds = _movieFocusNodes.keys.toList();
//     for (final movieId in nodeIds) {
//       try {
//         final node = _movieFocusNodes[movieId];
//         if (node != null) {
//           node.removeListener(() {});
//           node.dispose();
//         }
//       } catch (e) {
//         // Silent dispose error
//       }
//     }
//     _movieFocusNodes.clear();
//   }
...
//             FocusScope.of(context).requestFocus(_movieFocusNodes[firstMovieId]);
//             print('✅ Focus set to first movie (Index 0, Range 1)');
//           }
//         });
//       }
//     } catch (e) {
...
//   // Safe scroll to focused item
//   void _scrollToFocusedItem(String itemId) {
//     if (!mounted || _isDisposed) return;

//     try {
//       final focusNode = _movieFocusNodes[itemId];
//       if (focusNode != null && focusNode.hasFocus && focusNode.context != null) {
//         Scrollable.ensureVisible(
//           focusNode.context!,
//           alignment: 0.1,
//           duration: const Duration(milliseconds: 500),
...
//         );
//       } else {
//         // For non-YouTube videos, use your regular video player
//         await Navigator.push(
//           context,
//           PageRouteBuilder(
//             pageBuilder: (context, animation, secondaryAnimation) => 
...
//         loadingOverlay.remove();
//       } catch (overlayError) {
//         print('⚠️ [WARNING] Grid overlay removal failed: $overlayError');
//       }
//     }
...
//       ScaffoldMessenger.of(context).showSnackBar(
//         SnackBar(
//           content: const Text('Unable to play this movie. Please try another.'),
//           backgroundColor: Colors.red,
//           duration: const Duration(seconds: 3),
//         ),
//       );
...
//     // Check for YouTube ID (exactly 11 characters)
//     bool isYoutubeId = RegExp(r'^[a-zA-Z0-9_-]{11}$').hasMatch(url);
//     if (isYoutubeId) {
//       return true;
//     }
...
// // 🛡️ SAFE VIDEO DATA CREATION
// VideoData createSafeVideoData(Movie movie) {
//   try {
//     return VideoData(
//       id: movie.id.toString(),
//       title: movie.name.isNotEmpty ? movie.name : 'Unknown Movie',
//       youtubeUrl: movie.movieUrl,
//       thumbnail: movie.banner ?? movie.poster ?? '',
//       description: movie.description ?? '',
//     );
//   } catch (e) {
//     print('❌ [ERROR] Video data creation failed: $e');
...
//   void _retryLoading() {
//     if (!_isDisposed) {
//       // Reset everything and reload from beginning
//       setState(() {
//         _allMovies.clear();
//         _loadedRanges = {1};
...
//       if (_allMovies.isEmpty && !_isInitialLoading) {
...
//             // 🆕 Cumulative loading indicator
//             if (_isLoadingMore && _loadingRanges.isNotEmpty)
//               Positioned(
...
//                       const SizedBox(width: 8),
//                       Text(
//                         'Loading range ${_loadingRanges.first}...',
//                         style: const TextStyle(
//                           color: ProfessionalColors.accentGreen,
//                           fontSize: 12,
//                           fontWeight: FontWeight.w500,
...
//   // AppBar with range status
//   Widget _buildProfessionalAppBar() {
//     // Create range display string
//     String rangeDisplay = '';
//     if (_loadedRanges.isNotEmpty) {
//       final sortedRanges = _loadedRanges.toList()..sort();
...
//   // Movies Grid with cumulative loading
//   Widget _buildMoviesGrid() {
//     if (_isDisposed) {
//       return const SizedBox.shrink();
//     }
...
//     // Success State - Movies Grid with Cumulative Loading
//     return Focus(
//       autofocus: true,
//       onKey: (node, event) {
//         if (event is RawKeyDownEvent && !_isInitialLoading && !_isDisposed) {
//           if ([
//             LogicalKeyboardKey.arrowUp,
//             LogicalKeyboardKey.arrowDown,
//             LogicalKeyboardKey.arrowLeft,
...
//           } else if (event.logicalKey == LogicalKeyboardKey.enter ||
//                      event.logicalKey == LogicalKeyboardKey.select) {
//             if (_focusedIndex < _allMovies.length) {
//               _handleGridMovieTap(_allMovies[_focusedIndex]);
//             }
//             return KeyEventResult.handled;
//           }
//         }
...
//               animation: _staggerController,
//               builder: (context, child) {
//                 final delay = math.min((index / _allMovies.length) * 0.5, 0.8);
//                 final animationValue = Interval(
//                   delay,
...
// // 🆕 Cumulative Movie Card with range indicators
// class CumulativeMovieCard extends StatefulWidget {
//   final Movie movie;
//   final FocusNode focusNode;
//   final VoidCallback onTap;
//   final int index;
...
//   final int positionInRange;
//   final int totalRangesLoaded;
//   final bool isInLatestRange;

//   const CumulativeMovieCard({
//     Key? key,
//     required this.movie,
//     required this.focusNode,
//     required this.onTap,
...
//     required this.positionInRange,
//     required this.totalRangesLoaded,
//     required this.isInLatestRange,
//   }) : super(key: key);

//   @override
//   _CumulativeMovieCardState createState() => _CumulativeMovieCardState();
// }

// class _CumulativeMovieCardState extends State<CumulativeMovieCard>
//     with TickerProviderStateMixin {
//   late AnimationController _hoverController;
//   late AnimationController _glowController;
//   late Animation<double> _scaleAnimation;
...
//       widget.focusNode.removeListener(_handleFocusChange);
//     } catch (e) {
//       print('❌ Error disposing movie card: $e');
//     }
//     super.dispose();
//   }
...
//   // 🆕 Range indicator showing which range this movie belongs to
...
//         break;
//       case 2:
//         rangeColor = ProfessionalColors.accentGreen;
//         rangeText = 'R2';
//         break;
//       case 3:
//         rangeColor = ProfessionalColors.accentPurple;
//         rangeText = 'R3';
//         break;
//       case 4:
//         rangeColor = ProfessionalColors.accentOrange;
//         rangeText = 'R4';
//         break;
//       case 5:
//         rangeColor = ProfessionalColors.accentPink;
//         rangeText = 'R5';
//         break;
//       default:
...
//           border: Border.all(
//             color: rangeColor,
//             width: widget.isInLatestRange ? 2 : 1,
//           ),
//         ),
//         child: Text(
//           rangeText,

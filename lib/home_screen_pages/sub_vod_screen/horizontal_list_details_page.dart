...
class _HorizontalListDetailsPageState extends State<HorizontalListDetailsPage>
  ...
  // Focus management
  int focusedIndex = 0;
  List<FocusNode> genreFocusNodes = [];
  ...
  late Animation<double> _fadeAnimation;
  late Animation<Offset> _headerSlideAnimation;
  late Animation<double> _genreAnimation;
  ...
  void initState() {
    ...
    _initializeAnimations();
    _startAnimations();
    _fetchGenres(); // Fetch genres on init
  }

  void _initializeFocusNodes() {
    // Clear existing focus nodes
    for (var node in genreFocusNodes) {
      node.dispose();
    }
    genreFocusNodes.clear();
    
    // Create focus nodes for each genre
    for (int i = 0; i < genres.length; i++) {
      genreFocusNodes.add(FocusNode());
    }
    
    // Focus first button initially
    if (genreFocusNodes.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _initializeAnimations() {
    ...
    _genreAnimation = Tween<double>(
      begin: 0.0,
      end: 1.0,
    ).animate(CurvedAnimation(
      parent: _genreController,
      curve: Curves.easeOutCubic,
    ));
  }
  ...
  void _startAnimations() {
    ...
    // Start genre animation after a delay
    Future.delayed(Duration(milliseconds: 300), () {
      if (mounted) {
        _genreController.forward();
      }
    });
  }
  ...
  Future<void> _fetchGenres() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true && data['genres'] != null) {
          ...
          setState(() {
            ...
            // Set first genre as selected by default
          ...
          // Initialize focus nodes after genres are loaded
          _initializeFocusNodes();
          
          print('✅ Loaded ${genres.length} genres: $genres');
          print('✅ UI should now show genres buttons');
          
          // Start genre animation after data loads
          _genreController.forward();
      ...
      setState(() {
        isLoadingGenres = false;
        genres = ['Movies', 'Entertainment']; // Fallback genres
        selectedGenre = 'Movies';
        focusedIndex = 0;
      });
      
      // Initialize focus nodes for fallback genres
      _initializeFocusNodes();
  ...
  void _onGenreSelected(String genre, int index) {
    setState(() {
      selectedGenre = genre;
      focusedIndex = index;
    });
    print('🎯 Selected genre: $genre at index: $index');
    
    // Show selection feedback
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Selected: $genre'),
        backgroundColor: ProfessionalColors.accentGreen,
        behavior: SnackBarBehavior.floating,
  ...
  Widget _buildGenreSection() {
    ...
    return AnimatedBuilder(
      animation: _genreAnimation,
      builder: (context, child) {
        return Transform.translate(
          offset: Offset(0, (1 - _genreAnimation.value) * 50),
          child: Opacity(
            opacity: _genreAnimation.value,
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Fixed layout genre buttons (Wrap format)
                  Container(
                    width: double.infinity,
                    child: Wrap(
                      spacing: 10,
                          ...
                          onKey: (node, event) {
                            if (event is RawKeyDownEvent) {
                              if (event.logicalKey == LogicalKeyboardKey.arrowLeft && index > 0) {
                                ...
                                genreFocusNodes[focusedIndex].requestFocus();
                                return KeyEventResult.handled;
                              } else if (event.logicalKey == LogicalKeyboardKey.arrowRight && 
                                ...
                                // Navigate to previous row (approximately 4-5 buttons per row)
                                int itemsPerRow = (MediaQuery.of(context).size.width / 140).floor();
                                int newIndex = index - itemsPerRow;
                                if (newIndex >= 0) {
                                  setState(() {
                          ...
                          child: AnimatedContainer(
                            ...
                            child: Material(
                              ...
                              child: InkWell(
                                ...
                                onTap: () => _onGenreSelected(genre, index),
                                child: AnimatedContainer(
                                  duration: Duration(milliseconds: 300),
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    boxShadow: [
                                      ...
                                      if (isFocused)
                                        BoxShadow(
                                          color: ProfessionalColors.focusGlow
                                              .withOpacity(0.6),
                                          blurRadius: 12,
                                          offset: Offset(0, 0),
                                        ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // Enhanced title with channel ID info
                            ShaderMask(
                              shaderCallback: (bounds) => const LinearGradient(
                                colors: [
                                  ProfessionalColors.accentGreen,

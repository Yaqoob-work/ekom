...
class NetworkApi {
  ...
  @override
  int get hashCode => id.hashCode ^ name.hashCode ^ logo.hashCode ^ networksOrder.hashCode;
}
...
class MovieDetailsApi {
  ...
  @override
  int get hashCode => id.hashCode ^ name.hashCode ^ banner.hashCode ^ poster.hashCode ^ genres.hashCode ^ status.hashCode;
}
...
Future<List<NetworkApi>> fetchNetworks(BuildContext context) async {
  ...
  if (cachedNetworks != null) {
    try {
      ...
      networks = cachedBody.map((dynamic item) => NetworkApi.fromJson(item)).toList();
    } catch (e) {
...
Future<List<NewsItemModel>> fetchContent(
  ...
  if (cachedContent != null) {
    try {
      ...
      content.sort((a, b) => safeParseInt(a.index).compareTo(safeParseInt(b.index)));
    } catch (e) {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Sorting API data
      apiContent.sort((a, b) => safeParseInt(a.index).compareTo(safeParseInt(b.index)));
...
Future<Map<String, dynamic>> fetchMoviePlayLink(int movieId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load movie play link: ${response.statusCode}');
    }
  } catch (e) {
    rethrow;
  }
      ...
      ..onKey = (node, event) {
        if (event is RawKeyDownEvent) {
          if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
            ...
            context.read<FocusProvider>().requestManageMoviesFocus();  // Added missing parentheses
            return KeyEventResult.handled;
          }
        }
        return KeyEventResult.ignored;
      ...
      if (mounted) {
        setState(() {
          ...
          if (!_cacheLoaded || _networks.isEmpty) {
            _errorMessage = 'Failed to load networks: ${e.toString()}';
          }
        });
      }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (firstRowFocusNodes.containsKey(0)) {
        context.read<FocusProvider>().setFirstVodBannerFocusNode(firstRowFocusNodes[0]!);
...
class _ContentScreenState extends State<ContentScreen> {
  ...
  Future<void> _fetchContentInBackground() async {
    try {
      ...
      for (int i = 0; i < (fetchedContent.length > 2 ? 2 : fetchedContent.length); i++) {
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          String imageUrl = '';
          if (contentItem.poster.isNotEmpty && contentItem.poster != 'localImage') {
            imageUrl = contentItem.poster;
          } else if (contentItem.banner.isNotEmpty && contentItem.banner != 'localImage') {
            imageUrl = contentItem.banner;
          } else if (contentItem.image.isNotEmpty && contentItem.image != 'localImage') {
            imageUrl = contentItem.image;
          } else if (contentItem.thumbnail_high.isNotEmpty && contentItem.thumbnail_high != 'localImage') {
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Future<void> _updateUrlIfNeeded(Map<String, dynamic> playLink) async {
    String url = safeParseString(playLink['url']);
    String type = safeParseString(playLink['type']);
    
    if (type == 'Youtube' || type == 'YoutubeLive') {
      
      for (int i = 0; i < _maxRetries; i++) {
        if (!_shouldContinueLoading) break;
        
        try {
          String updatedUrl = await _socketService.getUpdatedUrl(url);
  ...
  Future<void> _playVideo(MovieDetailsApi movieDetails) async {
    ...
    try {
      ...
      final playLinkData = await fetchMoviePlayLink(widget.id);
      String originalUrl = safeParseString(playLinkData['url']);
      String playType = safeParseString(playLinkData['type']);
      
      if (originalUrl.isNotEmpty) {
        
        // Create mutable copy for URL updates
        Map<String, dynamic> mutablePlayLink = Map.from(playLinkData);
        
        int retryCount = 0;
        while (retryCount < 3) {
          try {
            await _updateUrlIfNeeded(mutablePlayLink);
            break;
          } catch (e) {
            retryCount++;
          ...
          await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => VideoScreen(
                videoUrl: safeParseString(mutablePlayLink['url']),
                videoId: widget.id,
                channelList: widget.channelList,
                videoType: safeParseString(mutablePlayLink['type']),
...
// NewsItemModel update (agar ye model file mein update nahi kiya hai)
// Ye NewsItemModel class ko bhi update karna hoga agar abhi tak nahi kiya hai:

/*
class NewsItemModel {
  final int id;        // String se int mein change
  final String name;
  final String banner;
  final String index;  // Ye string rakh sakte hain sorting ke liye
  final int status;    // String se int mein change
  ...
  @override
  int get hashCode => id.hashCode ^ name.hashCode ^ banner.hashCode ^ index.hashCode ^ status.hashCode;
}
*/
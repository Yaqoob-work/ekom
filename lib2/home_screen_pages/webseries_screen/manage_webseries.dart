...
// ✅ Main Professional WebSeries Widget (replacing BannerHorizontalList)
...
class _ProfessionalWebSeriesHorizontalListState
  ...
  //   targetOffset = math.max(0, targetOffset - screenPadding);
  ...
  //   print('🎯 Scrolling to position $index, offset: $targetOffset');
  // }
...
void _scrollToPosition(int index) {
  // Use Scrollable.ensureVisible for better scroll behavior
  if (index < webSeriesList.length && index < maxHorizontalItems) {
    ...
    if (webseriesFocusNodes.containsKey(webSeriesId)) {
      ...
      // Use Scrollable.ensureVisible with custom parameters
      Scrollable.ensureVisible(
        focusNode.context!,
        duration: AnimationTiming.scroll, // 800ms
        curve: Curves.easeInOutCubic,
        alignment: 0.03, // Keep item 20% from the left edge
  ...
  Future<void> fetchWebSeries() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          webSeriesList = jsonData.map((item) => WebSeriesModel.fromJson(item)).toList();
          isLoading = false;
        });
  ...
  Widget _buildWebSeriesList(double screenWidth, double screenHeight) {
    ...
    return FadeTransition(
      ...
      child: Container(
        ...
        // ✅ ViewAll से भी TV Shows पर भेजें
        setState(() {
          focusedIndex = -1;
          _hasReceivedFocusFromMovies = false;
        });
